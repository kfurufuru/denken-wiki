...
??? question "セルフチェック①: 直接埋設式で車両通行がある場所の埋設深さと、管路式で車両通行がない場所の埋設深さはそれぞれ何m以上か？"
    直接埋設・車両あり: **1.2m以上**。管路式・車両なし: **0.3m以上**。管路（管）による保護があるため管路式の方が浅くてよい。「深さは施設方式で変わる」が大原則。

??? question "セルフチェック②: 低圧電線と特別高圧電線が地中で交差する場合の離隔距離は何m以上か？"
