...
!!! info "📌 このページについて（2026-06-08 再編）"
    「単独運転の防止」は電技解釈 第8章に **独立した条文として存在しません**。用語は [第220条](../articles/kaishaku/220.md) で定義され、検出・解列の義務は **低圧＝[第227条](../articles/kaishaku/227.md)／高圧＝[第229条](../articles/kaishaku/229.md)** の系統連系用保護装置が規定します。本ページは旧 `kaishaku/232.md` の単独運転防止解説を **テーマ頁として再編** したものです。**以下の本文中に残る「第232条」表記は旧版の名残**で、単独運転防止に対応する独立条はありません（現在の[第232条](../articles/kaishaku/232.md)は「高圧・特別高圧連系における例外」）。
...
!!! note "📊 概要"
    ...
    **条文番号3階層**: 解釈 第232条（条のみ。項・号なしの単一条文）

> ==**単独運転**==（電力会社系統が停電したのに分散型電源だけが運転を続ける状態）は ==**作業者の感電事故**== ・ ==**系統再投入時の機器損傷**== を引き起こす。第232条は ==**単独運転防止装置**== の設置を義務付け、防止方式は ==**受動的方式**== と ==**能動的方式**== の **組み合わせ** が原則。
...
| **典拠（一次ソース）** | 電気設備の技術基準の解釈（令和7年11月版）第232条 — 経済産業省 |
...
!!! warning "混同注意：第228条（逆潮流）と第232条（単独運転防止）は別物"
    - **第228条 = 電力流出方向の制御**（逆潮流の有無を扱う）
    - **第232条 = 系統停電時の解列**（停電時の動作を扱う）
...
!!! tip "なぜこのマップを冒頭に置くか"
    過去14年の出題は **第220条〜第232条 群括り** で4回（R01／R03／R04下／R05上）。第232条単独より「連系設備一式の差分理解」が問われる。冒頭で隣接条文との **棲み分け** を整理し、複合問題で混同しない土台を作る。
...
| **第232条** | **単独運転の防止** | **受動的方式＋能動的方式** | **両方式の原理と組合せ理由** |

### 第232条の独自性
- **「単独運転」という現象** に対する **検出・解列義務** を規定（系統停電が前提条件）
- 防止方式は **受動的＋能動的の組合せが原則** — 単独運用では不感帯／誤動作リスク
- **逆潮流の有無に関わらず** 全連系設備に必須（第228条の「なし」でも第232条は適用される）

### 複合問題の典型パターン
1. **方式区別型**: 「受動的方式の例として正しいものはどれか」 → 電圧変動／周波数変動／位相跳躍
2. **組合せ理由型**: 「受動的方式だけでは不十分な理由を述べよ」 → 不感帯（発電量＝負荷バランス時の検出漏れ）
3. **逆潮流混同型**: 「逆潮流なしの設備で単独運転防止は不要か」 → 必要（第228条と第232条は独立義務）
...
<text x="400" y="26" text-anchor="middle" font-size="15" font-weight="700" fill="#212121">第232条 — 単独運転防止の2方式組合せ</text>
...
<text x="400" y="85" text-anchor="middle" font-size="14" font-weight="800" fill="#0d47a1">解釈第232条</text>
...
??? question "セルフチェック: 逆潮流なしの分散型電源では単独運転防止は不要か？"
    **誤り**。逆潮流の有無と単独運転発生は **無関係**。系統停電後、分散型電源が負荷とバランスして運転継続すれば逆潮流ゼロでも単独運転状態になる。条文第232条は **逆潮流に関わらず全連系設備に適用** され、第228条（逆潮流）とは独立した並行義務。

---

## 2. 条文原文

**電気設備の技術基準の解釈 第232条（単独運転の防止）**
...
→ 「**それぞれ一以上**」の表現は「両方式から最低1種ずつ」を意味する。受動的だけ／能動的だけでは条文不適合。

### 過去問で問われた／問われ得るキーワード

第232条は第220条〜第232条 群括りで過去14年計4回出題。穴埋め問題では条文中のキーワードが空欄になる。
...
!!! note "出典"
    電気設備の技術基準の解釈（令和7年11月版）第232条 — 経済産業省告示／JEAC 9701-2019 系統連系規程 第3章
...
| 「受動的方式及び能動的方式の **それぞれ一以上**」 | **両方式から最低1種ずつ**の義務 | 「いずれか一方で足りる」は誤り |
...
??? question "セルフチェック: 「能動的方式を1種類設置すれば条文要件を満たす」は正しいか？"
    **誤り**。条文は「受動的方式 **及び** 能動的方式の **それぞれ** 一以上」と明記。「及び」「それぞれ」が **両方式必須** を強制する。能動的方式単独は条文不適合。
...
!!! tip "暗記の核心：危険の本質"
    ...
    **シナリオ②：系統再投入時の機器損傷**
    ...
    この2大事故を防ぐため、**系統停電を検知したら直ちに解列** することが第232条の核心義務。
...
!!! warning "覚え方の比喩：医療診断の「2種類の検査を併用」と同じ"
    がん検診で **血液検査だけ** だと「初期がんを見逃す（不感帯）」、**画像検査だけ** だと「ノイズ陰影で誤検知（誤動作）」。両方併用して初めて「見逃しゼロ＋誤診ゼロ」に近づく。第232条の組合せ原則は、これと同じ **「弱点が逆向きの2つの検査の併用」** と理解できる。
...
<text x="400" y="338" text-anchor="middle" font-size="10" fill="#1b5e20">第232条が要求する「検出 → 解列」の連続フロー</text>
</svg></div>
...
| **転送遮断装置** | 電力会社系統側の遮断信号を分散型電源側に転送 | 大規模・特別高圧連系 | 単独運転検出の補完手段（第232条の上乗せ要件として要求される場合あり） |
...
| # | ❌ 誤った認識 | ✅ 正しい認識 |
|---|--------------|--------------|
| 1 | 受動的方式だけで条文要件を満たす | 条文は「**それぞれ一以上**」と明記。受動的のみは不感帯リスクで条文不適合 |
| 2 | 能動的方式だけで条文要件を満たす | 同上。**受動的＋能動的の組合せ**が必須（誤動作補完のため） |
...
| # | ❌ 誤った認識 | ✅ 正しい認識 |
|---|--------------|--------------|
| 4 | 逆潮流なし（第228条）なら単独運転防止（第232条）は不要 | **両者は独立義務**。逆潮流の有無に関わらず第232条は全連系設備に適用 |
...
| 10 | PCSがあれば自動的に第232条遵守 | **JET認証PCS**は要件を満たすが、高圧連系では追加で専用継電器の施設が必要 |
...
| 「逆潮流なしの設備では単独運転防止不要」 | 逆潮流の有無に関わらず **全連系設備に必須** | 第228条（逆潮流）と第232条（単独運転防止）は独立義務 |
| 「単独運転防止は高圧連系のみ必要」 | **低圧（第226条）・高圧（第229条）いずれも必須** | 連系形態を問わず適用 |
| 「能動的方式は系統健全時にも常に変動を生じさせる」 | 微変動は **電力会社が打ち消す** ため系統健全時は影響なし | 単独運転時のみ偏移が蓄積する仕組み |
| 「単独運転 = 逆潮流」 | **別概念**：単独運転 = 系統停電時の運転継続／逆潮流 = 通常時の電力流出方向 | 第232条と第228条は対象事象が異なる |
| 「JET認証PCSがあれば高圧連系でも第232条要件を満たす」 | 高圧連系では **専用継電器の追加施設が必要** | JET認証は低圧PCSの単体機能で、高圧連系の解列装置は別途必要 |
...
|------|---------|---------|------------|---------|
| **住宅用PV（10kW未満）** | 低圧100/200V | 第226条＋第232条 | JET認証PCS内蔵（受動的＋能動的） | PCS自動停止 |
| **小規模PV（50kW未満）** | 低圧 | 第226条＋第232条 | JET認証PCS or 専用継電器 | PCS停止 or 遮断器 |
| **高圧連系PV（500kW級）** | 高圧6.6kV | 第229条＋第232条 | UVR/UFR/OVR/OFR＋能動的継電器 | 連系遮断器の動作 |
| **メガソーラー（2MW級）** | 高圧/特別高圧 | 第229条＋第232条＋転送遮断 | 専用保護継電器盤＋転送遮断装置 | 専用遮断器＋系統側転送指令 |
| **風力発電** | 高圧 | 第229条＋第232条 | 同上＋風車制御の協調 | 連系遮断器＋風車停止 |

### 形態判定の思考プロセス

```
[分散型電源の規模・電圧] → [低圧 or 高圧 連系] → [第226条 or 第229条 適用] → [第232条 検出装置構成]
```

試験では「住宅用太陽光のPCSは何の認証が必要か」（→ JET認証）や「高圧連系で第232条を満たすには何が必要か」（→ 専用継電器）といった形で問われる。
...
!!! info "なぜこのセクションが必要か"
    第232条は **太陽光・風力の系統連系協議で必ず引用される** 条文。電気主任技術者の業務として、PCS出荷検査・系統連系協議書・保安規程の3場面で具体的に登場する。

### シーン① — PCS出荷検査・受入確認

事業用太陽光発電設備の **PCS（パワーコンディショナー）受入検査** で、第232条遵守を確認。
...
電力会社（一般送配電事業者）に提出する **系統連系協議書** で、第232条遵守を明記。
...
| 場面 | 必要書類・行為 | 第232条との関係 |
|------|--------------|-------------|
| PCS出荷検査 | JET認証・試験成績書 | 装置レベルでの第232条遵守確認 |
| 系統連系協議書 | 方式・整定値・解列時間の記載 | 系統運用主体への適合宣言 |
| 保安規程 | 定期点検項目・記録様式 | 運用継続中の第232条遵守担保 |
| 事故時対応 | 単独運転発生時のログ確認・原因分析 | 第232条不遵守事故の再発防止 |
...
!!! tip "なぜ3列翻訳が必要か"
    第232条は **法規用語と日常感覚にギャップが大きい** 条文（「単独運転」は日常語の「孤立稼働」、「解列」は日常語の「切り離し」）。試験では条文用語そのままで穴埋めされるため、日常感覚 → 法規用語 → 試験での問われ方の3列対応を整理しておくと混同を防げる。
...
| 逆潮流 | ぎゃくちょうりゅう | 分散型電源から系統へ電力が流れること（第228条の規制対象） | 第232条とは別概念。並行義務 |
...
- [電技解釈 第226条：低圧連系時の施設要件](../articles/kaishaku/226.md) — 低圧分散型電源の保護装置（第232条と組合せ適用）
- [電技解釈 第229条：高圧連系時の施設要件](../articles/kaishaku/229.md) — 高圧分散型電源の保護装置（第232条と組合せ適用）
...
- [ ] **解列装置**: PCS内蔵（低圧）／専用継電器（高圧）／転送遮断（特別高圧）

### 法令体系・隣接条文

- [ ] **第220条** = 用語定義（単独運転・分散型電源）
- [ ] **第226条** = 低圧連系の施設要件（第232条と組合せ）
- [ ] **第229条** = 高圧連系の施設要件（第232条と組合せ）
- [ ] **第228条** = 逆潮流（**第232条と並行義務・別概念**）
...
*最終確認: 2026-05-06 ｜ ステータス: v2.4（テンプレ全面準拠） ｜ 条文番号: ✅ 一次ソース照合済（令和7年11月版第232条） ｜ 出題実績: ✅ kakomon DB照合済（第220条〜第232条群4回） ｜ 数値根拠: — （本条は数値直接規定なし・施設要件規定） ｜ [バージョニング基準](../../reference/versioning.md) ｜ [テンプレv2.4仕様](../../reference/template-v24.md)*
