...
🟨 電気設備技術基準      ← 省令（技術的要件「何を守るか」）
🟩 電技解釈             ← 告示（具体的な適合方法「どうやって守るか」）
🟦 JIS・JEC 等          ← 規格（数値・試験方法）
...
> 🟩 **技術基準省令ファミリー**: 上の「電気設備技術基準」は技術基準省令の中核だが、発電方式別に **火技（発電用火力設備）・水技（発電用水力設備）・風技（発電用風力設備）・太陽電池設備（令和3年新設）** の各技術基準省令も電気事業法第39条に基づき定められている。電験3種では電技省令が中心だが、**風技は H24・H29・R04上 で穴埋め出題実績あり**。

??? question "セルフチェック: 5層ピラミッドを上から順に言えるか？"
    法律（電気事業法）→ 政令（施行令）→ 省令（施行規則・電気設備技術基準）→ 告示（電技解釈）→ 規格（JIS）。言えなかったらピラミッド図を再確認。
...
??? example "R02 問1 — 発電事業用の保安規程（施行規則第50条）｜正答: (1)ハ (2)ホ (3)ヲ (4)ヘ (5)ワ｜⭐⭐⭐難"
