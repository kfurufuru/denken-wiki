...
??? question "セルフチェック①: 需要率・負荷率・不等率の分母はそれぞれ何か。なぜその量が分母になるのか意味から説明せよ"
    需要率の分母=設備容量（全負荷の定格合計）。負荷率の分母=最大需要電力。不等率の分母=合成最大需要電力（同時最大値）。

    ただし **分母だけでは公式を復元できない**。複数需要家を合成した場面では **総合負荷率の分母も合成最大需要電力**（下の4公式表を参照）で、不等率の分母と一致する。分母が同じになったら分子で判別する — 分子が「各最大の合計」なら不等率、「合成平均需要電力」なら総合負荷率。
...
## 🧮 計算パターン別 セルフチェック { #patterns }

### パターン1（基本）: 需要率・不等率・負荷率を求める { #pattern1 }
...
??? question "問題: 設備容量500kW、需要率0.8、不等率1.25、力率0.9のとき、必要な変圧器容量[kVA]を求めよ"
    ...
    → 標準品から選ぶなら **500 kVA** を選定（356 kVA 以上で最小の標準容量）

    !!! danger "400 kVA は標準容量に存在しない"
        JIS C 4304（配電用6kV油入変圧器）の三相定格容量系列は 20/30/50/75/100/150/200/**300**/**500**/750/1000/1500/2000 kVA で、**400 kVA と 630 kVA は無い**。
        356 kVA の次は 300 を飛ばして **500 kVA**。系列に無い容量を答えると失点する。

    !!! warning "力率を忘れると kW のまま計算してしまう"
        変圧器容量は**kVA**。有効電力 [kW] を力率で割ってkVAに変換する。

### パターン3（複合）: 2工場合算の需要率・負荷率 { #pattern3 }
