...
1. 🔴致命 :dart: **絶縁靴と静電靴の使い分け**
    - 「どちらも安全のため」だが目的が正反対
    - 問題文の「環境条件」（可燃性ガスの有無、高圧作業かどうか）から判断する

2. 🟡要注意 :dart: **漏電遮断器の動作値と人体安全**
    - ELB: 30mA × 0.1秒 が標準
    - 高感度型: 15mA（水気の多い場所）
    - ELBがあっても接地工事は省略できない場合がある

3. 🟡要注意 :dart: **静電気の着火源としての危険性**
    - 最小着火エネルギー: 水素 0.02mJ、ガソリン蒸気 0.2mJ、粉じん 数十mJ
    - 人体の帯電エネルギー: 数mJ〜数十mJ → ガソリン蒸気は着火可能

4. 🟢安心 :dart: **特殊場所の施設との関連（条文3層整理）**
    - **人体保護の本体根拠**: **解釈第36条（地絡遮断装置の施設）** — 金属製外箱を有する使用電圧 **60V超**の低圧機械器具に接続する電路に、地絡時に自動遮断する装置を施設（※**解釈第37条は「避雷器等の施設」で無関係**。また **30mA・0.1秒は解釈の規定ではない**＝解釈全文に「30mA」の記載なし・内線規程/JIS 由来の実務標準値）
    - **背景（接地連動）**: 解釈第17条（C/D種接地。ELBがあれば500Ω緩和）
    - **対比軸（別射程の場所）**: 解釈第176条（可燃性ガス等の **危険場所** = 防爆構造の対象）
    - 危険場所では「防爆構造の電気機器 + 静電気対策」がセット。一般の漏電保護（**解釈第36条**）とは別レイヤ

5. 🟡要注意 :dart: **接地抵抗と感電の関係**
    - 接地抵抗が高い → 漏電時に機器外箱の電位が上昇 → 感電のリスク増大
    - D種接地工事: 100Ω以下（ELB施設時 500Ω以下）

??? question "セルフチェック①: 可燃性ガスが存在する作業場で電気作業をする場合、絶縁靴と静電靴のどちらを履くべきか？理由も答えよ。"
    静電靴を履く。可燃性ガス環境では人体の帯電→放電が爆発の着火源になるため、静電靴で帯電した電荷を大地に逃がす必要がある。絶縁靴を履くと電荷が逃げず帯電が蓄積し、放電時に爆発の危険がある。

??? question "セルフチェック②: 一般的な漏電遮断器（ELB）の感度電流と動作時間の標準値を答えよ。また水気の多い場所では何mAの高感度型を使うか。"
    標準値は感度電流30mA・動作時間0.1秒以内。水気の多い場所（浴室・厨房など皮膚抵抗が低下する場所）では15mA（高感度型）を使用する。

## :link: 関連テーマ（役割別）

- **対策連動**：[接地工事](setsuchi.md) — 漏電保護（ELB）と接地（C/D種）は連動。本ページの500Ω緩和は両者がセット
- **同根拠（ELB詳細）**：[保護装置（過電流・地絡）](hogo-sochi.md) — 漏電遮断器（ELB）の動作原理・整定値の詳細
- **対比（別射程・危険場所）**：[特殊場所の施設](tokushu-basho.md) — 解釈第176条の防爆環境。一般の漏電保護とは別レイヤの規定
