...
??? question "セルフチェック①: IV電線（1.6mm）を同一管内に4本収納した場合の実効許容電流は何Aか。本数が増えると低減率を掛けるのはなぜか"
    19A × 0.63 ≒ **12A**。管内3本以下なら0.70倍だが、4本は0.63倍に下がる点がポイント。問題文で本数を確認する癖をつけること。

??? question "セルフチェック②: ビニルコードを屋内の固定配線に使用することはできるか？"
...
1. 🔴致命 :dart: **許容電流はカタログ値ではなく「低減後の実効値」で判断する**
...
2. 🔴致命 :dart: **電線の種類と使用できる場所・用途は1対1で決まっている**
    - **OW線（屋外用）を屋内配線に使う → 不適切** ✅
    - **DV線（引込用）を一般の架空電線路に使う → 不適切** ✅
    - **ビニルコードを固定配線に使う → 禁止（移動電線専用）**
    - **「この電線はどこで使えるか」を先に確認する**

3. 🟡要注意 :dart: **引込線と引込口配線の定義を混同しない**
    - **引込線 = 電柱の分岐点 → 建物の引込口（屋外）**
    - **引込口配線 = 引込口 → メーター（屋内）** ✅
    - 「引込線」を問われているのに「引込口配線」の知識で答えてしまうミスが頻出

4. 🔴致命 :dart: **CVとIVの許容電流の違い — 耐熱温度が根拠**
    - **IV：耐熱温度60℃ → 許容電流が小さい**
    - **CV：耐熱温度90℃ → 許容電流が大きい**（同じサイズでもCVの方が流せる）✅
    - 幹線設計でCVを選ぶ理由は「許容電流の余裕」が主な理由
    - **「温度が高いほど大電流OK」という物理直感を持つ**

5. 🟡要注意 :dart: **電線サイズと分岐回路の対応を覚えているか**
    - **1.6mm → 15A・20A回路用（19A）** ✅
    - **2.0mm → 20A回路用（24A）** ✅
    - **2.6mm → 30A回路用（33A）** ✅
    - **サイズ→許容電流→回路定格の3点セットで覚える**

---

## 📝 出題実績

{{ render_freq_summary('densen-cable') }}

| 年度 | 問 | 形式 | 何が問われたか | 条文 |
|------|----|------|--------------|------|
| R05上 | 問8 | 穴埋 | 電気使用場所での配線の使用電線 | 省令第57条 |
| R04下 | 問11 | 計算 | 高圧架空電線の引張強さに対する安全率と許容引張荷重 | 解釈第66条 |
| H27 | 問12 | 計算 | 電線の許容電流 | — |
| H25 | 問3 | 穴埋 | 電気使用場所での配線の使用電線 | 省令第57条 |

**出題頻度**: 過去問 SoT（`_data/kakomon.yml`・H23〜R07下 全247問）で theme=`densen-cable` は **4問**
...
!!! info "関連テーマでの出題（SoT では別テーマに登録）"
    ...
    |------|----|---------|------|-----------|
    | H27 | 問2 | 電気用品安全法に基づく電気用品の電線 | PSE法 | `jigyoho-taikei` |
...
!!! note "出題実績の SoT 再構成（2026-08-29）"
    ...
    - **追加 3件**: R05上問8（電気使用場所での配線の使用電線）／R04下問11（高圧架空電線の引張強さに対する安全率と許容引張荷重）／H27問12（電線の許容電流）
    - **記述を SoT へ是正 1件**: H25問3「引込線の施設・電線の種類」→「電気使用場所での配線の使用電線」 — いずれも旧記述の条文番号が SoT と一致しなかったもの
...
!!! note "出題実績の是正（2026-06-13）"
    - 旧掲載「R02 問7＝電線・ケーブルの種類と施設方法」を削除。実際の R02 問7 は引込線における用語の定義（解釈第1条）であり、電線・ケーブルの種類とは無関係な合成誤りのため。
