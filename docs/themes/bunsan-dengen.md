...
| 電圧変動の制限 | 連系点の電圧を適切な範囲に保つ ⚠️ 未確認（「±2%以内」の根拠条文未特定。低圧の供給電圧は101±6V / 202±20Vで管理。瞬時変動は常時電圧の±10%以内） | 分散型電源の出力変動に伴う配電線の電圧上昇・降下を許容範囲内に収める |
...
| **単独運転検出** | 受動的 + 能動的の両方式（各1方式以上）⚠️ 未確認（第227条では両方必要。「受動のみ」は誤り。試験問題での扱い要注意） | 受動的 + 能動的の両方式 | 受動的 + 能動的の両方式 + 転送遮断 ✅ |
| **逆潮流なし時のRPR** | 必要 | 必要 | 必要 |
...
| **電圧変動許容値** | 101±6V / 202±20V以内（系統連系技術要件） ⚠️ 未確認（「±2%」表記の根拠未特定） | ⚠️ 未確認（根拠未特定） | 個別協議 ⚠️ 未確認 |
...
!!! warning "低圧も受動的+能動的の両方式が必要（第227条）"
    電技解釈第227条では「受動的方式及び能動的方式のそれぞれ1方式以上を含む」と規定。「低圧は受動的のみ」は誤り。ただし、試験の選択肢では表現が異なる場合があるため、過去問で実際の出題パターンを要確認。 ⚠️ 未確認（試験での問われ方）
...
4. :dart: **連系点の電圧変動と力率要件**
    ...
    - 電圧変動の許容値: 低圧供給電圧は101±6V / 202±20V以内（電技解釈）。「±2%」という表記の根拠は未特定 ⚠️ 未確認
...
---
*最終確認: 2026-03-29 WebSearch検証 | ステータス: v0.7 検証済み（確認率81%: 21/26箇所 ✅ ） | [バージョニング基準](../reference/versioning.md)*
