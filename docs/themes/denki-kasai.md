...
| 絶縁抵抗の区分判断 | どちらか一方の電圧だけで判断しようとする | **使用電圧300V／対地電圧150V の2軸**で判断する（省令第58条：使用電圧300V以下かつ対地電圧150V以下→0.1／使用電圧300V以下かつ対地電圧150V超→0.2／使用電圧300V超→0.4MΩ） |
...
2. 🟡要注意 :dart: **過電流遮断器の動作時間の数値**
    - 「30A以下: 1.25倍→60分、2倍→2分」は頻出。定格電流のランクごとに時間が変わることに注意
    - 誤答パターン: 30A超のランクと混同する

3. 🟡要注意 :dart: **漏電遮断器の感度電流と動作時間**
    - 30mA・0.1秒は標準値。水気のある場所は15mA（高感度型）
    - 誤答パターン: 動作時間を「0.1秒以内」ではなく「0.1秒以上」と逆に覚えてしまう

4. 🟡要注意 :dart: **トラッキングの発生条件**
    - 「埃 + 湿気」の両方が必要。どちらか一方だけでは発生しにくい
    - 誤答パターン: 「濡れれば発生する」と湿気だけに注目する

5. 🟢安心 :dart: **ELBは感電防止だけでなく電気火災防止も目的**
    - ELBの設置義務の根拠を「感電防止だけ」と理解していると、火災防止観点の問いで迷う

---

??? question "セルフチェック①: 絶縁抵抗の区分が電圧の高い区分ほど基準値も大きい（150V以下0.1MΩ→300V超0.4MΩ）のはなぜか。判断に使う電圧は「使用電圧」か「対地電圧」か。"
    **電圧が高いほど絶縁が破れたときの漏れ電流・感電・発熱が大きくなるため、より高い絶縁抵抗を要求して漏れ電流を同程度に抑える。** 区分の判断は対地電圧（使用電圧ではない）。結果として 150V以下: 0.1MΩ以上 / 150V超〜300V以下: 0.2MΩ以上 / 300V超: 0.4MΩ以上。

??? question "セルフチェック②: 同じ過電流遮断器（30A以下）でも、1.25倍の電流より2倍の電流の方が許容動作時間が短い（60分→2分）のはなぜか。"
    **電流が大きいほど電線の発熱（I²t）が急増し絶縁が早く焼損するため、大電流ほど速く切る必要がある。** 動作時間は「過電流の大きさ＝危険の急進度」で決まる。結果として 30A以下では 1.25倍→60分以内 / 2倍→2分以内。

## :link: 関連テーマ

- [保護装置（過電流・地絡）](hogo-sochi.md) — 過電流遮断器・漏電遮断器の動作原理と設置要件
- [絶縁抵抗・絶縁耐力](zetsuen.md) — 絶縁測定の方法と判定基準
