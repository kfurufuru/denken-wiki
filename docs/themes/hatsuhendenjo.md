...
| 「随時巡回方式」 | 定期的に人が見に行く（週1回など ⚠️） | センサーで常時監視はしない。異常が発生しても巡回まで発見できない前提で設備冗長度を高める |
...
|---------|------------|----------------|-------------------|
| 自動停止装置 | 必要 | 必要 | 必要 |
| 異常時の警報装置 | 不要 ⚠️ | 必要（警報を巡回員へ） | 必要（制御所へ常時伝送） |
| 遠隔制御設備 | 不要 | 必要（操作可） | 必要（常時操作可） |
| 通信回線 | 不要 | 必要（電話等） ⚠️ | 必要（常時接続） |
| 施錠その他の立入防止 | 必要 | 必要 | 必要 |
...
| 要件 | 内容 | 根拠条文 |
|------|------|---------|
| 柵・塀の設置 | 高さ2m以上（特別高圧の場合は電圧に応じた離隔を確保） ⚠️ 未確認 | 省令第23条・解釈第38条 |
| 施錠・鍵管理 | 取扱者以外が容易に開閉できない構造 | 解釈第38条 |
| 危険表示 | 「高電圧危険」等の表示板の設置 | 解釈第38条 |
| 充電部との離隔 | 柵・塀から充電部まで電圧に応じた離隔距離 ⚠️ 未確認 | 省令第24条 |
...
| 機器種別 | 防油設備 | 消火設備 | 根拠条文 |
|---------|---------|---------|---------|
| 油入変圧器（一定規模以上）| 防油堤または収油設備 | 自動消火設備（水噴霧・泡等）⚠️ | 解釈第45条 ⚠️ |
| 油入開閉器 | 防油堤または収油設備 | 必要に応じて ⚠️ | 解釈第45条 ⚠️ |
| 小規模油入機器 | 不燃材料の床+排油口 ⚠️ | 不要の場合あり ⚠️ | 解釈第45条 ⚠️ |
...
3. :dart: **危険表示の要否を見落とす**
    - 発変電所の構内には「高電圧危険」等の危険表示が必要（解釈第38条）⚠️
    - 柵・塀を設置しただけでは不足。表示板と施錠のセットが要件
    - 試験では「柵の設置のみで正しい」という引っ掛けが出る ⚠️

4. :dart: **油入機器の防火要件を過小評価する**
    - 油入変圧器・油入開閉器には**防油堤（または収油設備）**が必要
    - さらに一定規模以上では**自動消火設備**も要求される ⚠️
...
| 年度 | 問 | 形式 | 何が問われたか | 条文 |
|------|----|------|--------------|------|
| H28 | 問6 | 択一 | 常時監視をしない発電所の施設要件（監視方式の種別） ⚠️ 未確認 | 省令第46条 |
| H29 | 問6 | 択一 | 発変電所への取扱者以外の立入防止（柵・塀・施錠） ⚠️ 未確認 | 省令第23条・解釈第38条 |
| H30 | 問6 | 択一 | 常時監視をしない発電所の条件（随時巡回・遠隔常時） ⚠️ 未確認 | 省令第46条 |
| H31(R01) | 問6 | 択一 | 発変電所の施設条件（変圧器・油入機器の防火）⚠️ 未確認 | 解釈第45条〜第48条 |
| R02 | 問6 | 択一 | 発電機の保護装置の種類と対象 ⚠️ 未確認 | 解釈第39条〜第44条 |
| R03 | 問6 | 択一 | 監視方式（随時監視制御方式の設備要件）⚠️ 未確認 | 省令第46条 |
| R04 | 問6 | 択一 | 変電所の施設（GIS・母線保護・消火設備）⚠️ 未確認 | 解釈第45条〜第48条 |
| R05 | 問6 | 択一 | 発変電所の立入防止と監視方式の複合問題 ⚠️ 未確認 | 省令第23条・第46条 |
| H27 | 問6 | 択一 | 発変電所の取扱者以外の立入防止（柵・塀の高さ） ⚠️ 未確認 | 省令第23条 |
| H23 | 問5 | 択一 | 発電機の保護装置 ⚠️ 未確認 | 解釈第39条〜第44条 |
