...
| R03 | 問5 | 穴埋 | 発電機の保護装置の動作対象（過電流・電圧異常・冷却水断・軸受温度） | 解釈第42条 |
| R01 | 問7 | 穴埋 | 常時監視をしない発電所の施設（監視方式） | 解釈第47条・第48条 |
| H30 | 問6 | 穴埋 | 発電所等への取扱者以外の者の立入の禁止（さく・へい・施錠） | 解釈第38条 |
| H29 | 問4 | 穴埋 | ガス絶縁機器等の危険の防止 | 省令第33条 |
| H29 | 問5 | 穴埋 | 風車の安全な状態の確保 | 風力省令 |
| H27 | 問6 | 穴埋 | 常時監視をしない発電所の施設要件（監視方式の種別） | 解釈第47条・第48条 |
| H24 | 問4 | 穴埋 | 発電用風力設備に関する技術基準（風車の支持物） | 風力省令 |
| H23 | 問5 | 択一 | 常時監視をしない発電所等の施設 | 解釈第47条・第48条 |
...
!!! note "出題実績テーブルの全行是正（2026-06-13）"
    旧版は10行中7行が別問題の論点・条文の合成だった（例: 「H31(R01)問6＝油入機器の防火」→ R01問6 は接地工事で H31 年度区分は不存在／「R02問6＝発電機保護」→ 実際は屋内配線工事の種類／「R04問6・R05問6」→ 上期下期分離前の合成）。kakomon.yml の theme=hatsuhendenjo 登録6問で再構成済み（denken-ou 照合）。
