...
> 🧠 **線の記憶ポイント**: 法43条「選任せよ」→施行規則「届出せよ」。**法42条（保安規程=文書）とセット**で毎年出題。三種は「5万V未満」かつ「5,000kW未満」の2条件。
...
| 外部委託 | ==7,000V以下==で受電する需要設備等 ✅ | 所轄の産業保安監督部長の==承認== ✅ |
| 兼任 | 常時勤務する事業場＋近隣の事業場 ✅ | 所轄の産業保安監督部長の==承認== ✅ |
...
| 項目 | 要件 |
|------|------|
| 対象設備 | ==7,000V以下==で受電する需要設備（自家用電気工作物）✅ |
| 委託先 | ==電気保安法人== または ==電気管理技術者== ✅ |
| 点検頻度 | ==月次点検1回以上==（隔月の特例あり）✅ |
| 年次点検 | ==年1回以上==の停電点検 ✅ |
| 換算装置数 | 電気管理技術者1人あたり==33ポイント以内== ✅ |
...
??? example "R02 問1（穴埋）— 主任技術者の職務・監督範囲 ｜ 法43条"
    ...
    ??? success "解答・解説"
        **正解: (1)**　（ア）監督　（イ）指示　（ウ）50,000　（エ）5,000
        ...
        | （イ） | **指示** | 法43条4項「指示に従わなければならない」。「命令」は使われない。指示≠命令（従事者は指示を受ける立場） |
    ...
    **(1)** (a)のみ正しい　**(2)** (b)のみ正しい　**(3)** (a)(b)が正しい　**(4)** (b)(c)が正しい　**(5)** (a)(b)(c)全て正しい
    ...
    ??? success "解答・解説"
        **正解: (5)** — (a)(b)(c)全て正しい
        ...
        | 選択肢 | 判定 | 根拠条文 |
        |--------|------|---------|
        | (a) 選任→届出義務 | ✅ | 法43条2項。選任したときは**遅滞なく**届け出る（事前届出ではない）|
        | (b) 誠実義務 | ✅ | 法43条3項の誠実義務規定 |
        | (c) 指示遵守義務 | ✅ | 法43条4項。従事者は主任技術者の指示に従う義務がある |
