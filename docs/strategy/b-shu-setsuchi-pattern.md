...
| 🟢 軽度 | D種接地と混同 | D種は **100**Ω（0.5秒以下なら500Ω）。B種は **150/Ig** 系 |
...
- [解釈第17条](../articles/kaishaku/17.md) — B種接地工事の本体規定（150/300/600の3段階緩和）。本ページの 4ステップは全てこの条文から出る

#### 背景（成立条件・上位ルール）

- 省令第10条 — 電気設備の接地（原則規定）。第17条の数値が決まる前提
- 解釈第24条 — 高圧又は特別高圧と低圧との混触による危険防止施設（B種が「混触防止接地」と呼ばれる根拠）

#### 拡張（接地極の代替）

- 解釈第18条 — 工作物の金属体を利用した接地工事（B種の接地極代替）

#### 対比（別パターン）

- [非接地テブナンパターン](hiseichi-thevenin-pattern.md) — 非接地系統では本ページの150/Ig が成立しない。中性点電位と地絡電流をテブナンで解く別解法
