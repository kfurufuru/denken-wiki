...
| **混同注意** | 同番号の **電気事業法第21条**／**[解釈第21条（高圧の機械器具の施設）](../kaishaku/21.md)** とは **別物**（特に解釈§21 取り違えの誤記事件 2026-05-05 が起きており要警戒）。本条は「電技省令」第21条（架空・地中電線の感電防止） |
...
- **2026-05-05（v2.2 UX改修）**: ユーザフィードバックに基づき2点改善。(1) §4 に絶縁電線 vs ケーブル の **断面図SVG**（同心円構造）を追加し、シース（外被）の有無が地中可否の境界であることを視覚化。(2) §5 判定フローを **mermaid → SVG** に置き換え、「施設箇所→電圧→例外」の3ステップ構造と「地中＝即ケーブル／架空＝2段階確認」の判断軸を明示。
- **数値検証判定: N/A**（原則規定・材料規定のため数値テーブルなし）
- **2026-05-05 v2.3 テンプレートv2.3移行**: メタテーブルに「典拠／照合日／隣接条文／混同注意」4行追加。フッタにステータスラベル3軸（条文番号／出題実績／数値根拠）導入。**特に解釈§21 取り違え誤記事件（2026-05-05）の教訓を反映し、混同注意行で省令§21 vs 解釈§21 を明示分離**。
    - **条文番号確定根拠**: 電技省令 平成九年通商産業省令第五十二号 令和5年改正版（e-Gov API lawid=409M50000400052・2026-05-03取得済）
    - **既存wiki参照の独立検証**: 実施（隣接 §20・§22 と内容重複なきこと確認・本条は「架空＝2択／地中＝ケーブル一択」固有内容）
