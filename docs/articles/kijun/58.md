...
| 機器 | 別名・略称 | 動作 | 検出対象 | 第58条との関係 |
|------|----------|------|---------|-------------|
| **開閉器** | スイッチ、ナイフスイッチ | **手動**で電路を開閉 | なし（手動操作） | 測定の区切り単位（条文に明記） |
| **過電流遮断器** | 配線用遮断器（MCCB）、ブレーカー | **自動**で電路を遮断 | **過電流**（短絡・過負荷） | 測定の区切り単位（条文に明記） |
| **漏電遮断器** | ELB、ELCB、漏電ブレーカー | **自動**で電路を遮断 | **漏えい電流**（地絡） | 条文には登場しない（ひっかけ選択肢） |
...
- **「低圧は一律0.1**MΩ**でOK」** — 低圧電路の絶縁抵抗値は3段階に分かれる。0.1MΩが適用されるのは対地電圧150V以下の場合のみ。300V以下で0.2MΩ、300V超で0.4MΩと区分される
...
- **「0.2**MΩ**と0.4**MΩ**は使用電圧だけで決まる」** — 使用電圧300V以下/超の境界に加え、300V以下の中で対地電圧150V以下かどうかの境界もある。2軸で判定する必要がある
