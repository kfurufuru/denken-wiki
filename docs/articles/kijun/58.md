...
!!! note "📊 試験対策メタ"
    ...
    **直近出題**: R07下 問5（穴埋）／R07上 問3・R06上 問8・R02 問3・H26 問6（穴埋）／H24 問5（論説）
...
| 基準値 | 公式根拠 | 参考計算 |
|------|-----------|----------|
| **0.1MΩ** @対地100V | 漏えい電流 **1mA**（公式の基準ケース） | 100V ÷ 0.1MΩ = **1.0mA** |
| **0.2MΩ** @対地200V | 1mA以下（公式基準） | 200V ÷ 0.2MΩ = **1.0mA** |
| **0.4MΩ** @対地300V超 | 1mA以下（公式基準） | 300V ÷ 0.4MΩ = **0.75mA** |
...
過電流遮断器とは、過電流を検出して自動的に電路を遮断する機器のこと。代表例は**配線用遮断器（ブレーカー）**。開閉器（スイッチ）は手動で開閉する機器で、自動遮断機能はない。この2つがセットで条文に登場する。
...
- **「低圧は一律0.1MΩでOK」** — 低圧電路の絶縁抵抗値は3段階に分かれる。0.1MΩが適用されるのは対地電圧150V以下の場合のみ。300V以下で0.2MΩ、300V超で0.4MΩと区分される
- **「電路全体で一度測定すればOK」** — 開閉器または過電流遮断器で区切れる電路ごとに個別測定が必要。分電盤のブレーカー単位で測定するイメージを持つこと
- **「電線相互間だけ測れば十分」** — 条文には「電線相互間**及び**電路と大地との間」と明記されている。相互間と対地間の**両方**が必須

### 🟡 注意（出題頻度高い）

- **「0.2MΩと0.4MΩは使用電圧だけで決まる」** — 使用電圧300V以下/超の境界に加え、300V以下の中で対地電圧150V以下かどうかの境界もある。2軸で判定する必要がある
- **「漏えい電流1.5mA以下なら絶縁抵抗測定と同等」** — 解釈第14条が定める公式基準は**1mA以下**。1.5mAは不正解の選択肢として頻出する
- **「基準値以上なら絶対安全」** — 基準値はあくまで最低ライン。実務では経年劣化による抵抗値の低下傾向を継続的に観察することが重要

### 🟢 軽微（余裕があれば）

- **「メガー測定は数秒で完了する」** — 正確な測定には1分以上かけて指示値が安定するのを確認する必要がある。短時間の値は誘電吸収の影響で不安定
- **「メガーは1000Vや2500V印加が標準」** — 低圧電路の絶縁抵抗測定では直流500V印加が標準（JIS C 1302）。高電圧印加は高圧・特別高圧電路向け
...
!!! note "R08 出題予測"
    「対地電圧150V以下なら0.1MΩ」というひっかけが定番。**3段階基準値（0.1/0.2/0.4MΩ）の数値暗記＋電線相互間/対地間の両方測定** の2点は必ず押さえる。
...
| 過電流遮断器 | かでんりゅうしゃだんき | 過電流を検出して自動遮断する機器。配線用遮断器（ブレーカー）が代表例 | 「開閉器又は過電流遮断器」のセットで出題される |
...
*最終確認: 2026-05-04 | 数値検証完了: 2026-05-02（CONDITIONAL PASS・致命数値2件修正済） | ステータス: v2.0（全14セクション完備・12項目品質改修済） | [バージョニング基準](../../reference/versioning.md)*
