...
### なぜ二号は「高さ＋距離の和」で5m以上か

人体寸法（身長 ≒ 1.7m＋腕のリーチ ≒ 0.7m）から **手の届く最大範囲は約2.4m**。脚立等を考慮しても5m以上あれば実用上届かない。
...
| 区域 | 高さ | 想定リスク |
|------|------|----------|
| **市街地** | **4.5m** 以上 | 一般人・建物・脚立等の接触リスク高 → 厳しめ |
| **市街地外** | **4m** 以上 | 農村等で人口密度低 → 0.5m緩和 |

工場敷地外の **柱上変圧器** が市街地と市街地外で別寸法になる根拠。実務では電力会社の架線設計基準と整合する。
...
*最終確認: 2026-05-13 | ステータス: v1.7（可読性改善・「・」整理＋事例SVG追加）｜ 条文番号: ✅ 一次ソース3点照合済 ｜ 出題実績: ✅ kakomon DB 4件照合済 ｜ 数値根拠: ⚠️ 学習教材ベース（5m/4.5m/4m の安全距離由来は実用設計値、公式数学的導出は不明）／第22条22-1表は電験王1 R03問2解説で照合済 ｜ 隣接条文: ⚠️ 第20条タイトルは19.md準拠、経産省告示原本での完全照合は[要再確認] ｜ [バージョニング基準](../../reference/versioning.md)*
