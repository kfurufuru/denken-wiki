...
4. **計算型（B種）**: 「1線地絡電流 Ig=4A、自動遮断装置 1秒以内動作の場合のB種接地抵抗」 → **600÷4 = 150Ω以下**（1秒以下動作なので **600/Ig** を適用。1秒超〜2秒以下なら 300/Ig、緩和なしなら 150/Ig）
...
??? question "セルフチェック: 高圧受電設備の外箱＝A種接地（種別は何で決まるか）"
    ...
    **接地工事の種別は「対象機器の電圧（＝危険度）と混触リスク」で決まり、高圧機器の外箱は最も危険度が高いため A種になるから。** 高圧機器は地絡時の外箱の電位上昇が大きく、感電すれば致命的になりうる。そのため最も厳しい A種（接地抵抗 **10Ω以下**／接地線 直径 **2.6mm以上の軟銅線**＝引張強さ1.04kN以上）を課す。数値はこの「高圧＝高危険度」という位置づけから導かれる結果。
