...
diagnostic:
  ...
  exclude_branch_table: false       # A種/B種/C種/D種の4分岐構造あり（B種のみ17-1表/17-3表・他は条文本文）
...
!!! note "📊 概要"
    ...
    **直近出題**: R07下期 問4（電気設備の接地）／ R07下期 問11（接地極の埋設深さ及び接地抵抗測定）

> 第17条は **接地工事の本体規定** であり、**A種・B種・C種・D種の4種類** すべてを規定する。各種の **接地抵抗値** と **接地線太さ** はそのまま試験に問われる中核数値（原典で表になっているのは **B種のみ**＝17-1表・17-3表。A種・C種・D種の数値は条文本文で規定）。第18条（工作物の金属体を接地極化）・第19条（電路の接地）はいずれも本条を補完する位置づけであり、**接地工事の種別・抵抗値・線太さの根拠条文は本第17条** であることを最初に押さえる。
...
> **このセクションは 第2条 条文原文の接地抵抗値・接地線太さ（原典表はB種のみ＝17-1表・17-3表。A・C・D種は条文本文）を1表に圧縮した受験者向けサマリー**。条文準拠の正式な表・要旨は **[第2条 条文原文（17-1表・17-3表）](#2-17-117-3)** を参照。

| 種別 | 主な対象 | 接地抵抗値 | 接地線 | 試験での決め手 |
|---|---|---:|---:|---|
| **A種** | 高圧・特別高圧機器の金属製外箱等 | **10**Ω以下 | **2.6mm以上** | 高圧・特高機器 |
| **B種** | 高圧・特高と低圧を結合する変圧器の低圧側中性点等 | **150/Ig Ω以下** | **2.6mm or 4mm以上** | 混触防止 |
| **C種** | 低圧 **300V超過** の機器外箱等 | **10**Ω以下 | **1.6mm以上** | 使用電圧300V超 |
| **D種** | 低圧 **300V以下** の機器外箱等 | **100**Ω以下 | **1.6mm以上** | 使用電圧300V以下 |

> 迷ったら「A＝高圧・特高」「B＝変圧器の混触防止」「C＝低圧300V超」「D＝低圧300V以下」で判定する。詳細な数値・線径・緩和規定は **第2条 条文原文**（条文準拠）と 第9条（A/B 2層暗記表）を参照。
...
| 第18条 | **工作物の金属体を接地極に流用** | 鉄骨等の利用（**第1項＝共用接地極（A〜D種含む）／第2項＝2Ω以下のA・B種接地極特例／第3項＝接地線特例**） | 大地抵抗 **2**Ω以下（第2項）・**等電位ボンディング**・**50V接触電圧**・**1線地絡電流** |
...
### 複合問題の典型パターン
1. **種別判定型**: 「対象機器の電圧から A〜D種のどれか」 → 各種接地工事の適用対象（条文本文・本記事「適用対象（要旨）」表）で判定
2. **数値暗記型**: 「C種接地工事に漏電遮断器を施設した場合の接地抵抗値は」 → **500**Ω以下
3. **線太さ判定型**: 「B種接地工事のうち高圧と低圧を結合する変圧器の場合の最小直径は」 → **2.6mm以上**（17-3表）
4. **計算型（B種）**: 「1線地絡電流 Ig=4A、自動遮断装置 1秒以内動作の場合のB種接地抵抗」 → **600÷4 = 150**Ω以下（1秒以下動作なので **600/Ig** を適用。1秒超〜2秒以下なら 300/Ig、緩和なしなら 150/Ig）
...
- **A種**: 高圧・特別高圧機器の外箱 → 抵抗 **10**Ω以下／線径 **2.6mm以上**
...
- **C種**: 300V超の低圧機器外箱 → 抵抗 **10**Ω以下（漏電遮断器0.5秒以内 **500**Ω以下）／線径 **1.6mm以上**
- **D種**: 300V以下の低圧機器外箱 → 抵抗 **100**Ω以下（漏電遮断器0.5秒以内 **500**Ω以下）／線径 **1.6mm以上**
- なぜ4種類か: 対象機器の **危険度（電圧の高さ）** と **混触リスク** に応じて段階的に厳しさを変える設計
- A種10Ωの根拠: **機器外箱の電位上昇抑制 + 高圧地絡継電器（GR）動作確保**（B種の混触防止計算式と混同しない）

??? question "セルフチェック: 高圧受電設備の外箱＝A種接地（種別は何で決まるか）"
    Q: 高圧受電設備の金属製外箱に **A種** 接地工事を課すのはなぜか。接地工事の種別は何で決まるか。

    **接地工事の種別は「対象機器の電圧（＝危険度）と混触リスク」で決まり、高圧機器の外箱は最も危険度が高いため A種になるから。** 高圧機器は地絡時の外箱の電位上昇が大きく、感電すれば致命的になりうる。そのため最も厳しい A種（接地抵抗 **10**Ω以下／接地線 直径 **2.6mm以上の軟銅線**＝引張強さ1.04kN以上）を課す。数値はこの「高圧＝高危険度」という位置づけから導かれる結果。
...
!!! note "原典の「17-1表」はB種専用"
    原典（電技解釈 令和7年11月版）で「17-1表」と呼ばれるのは **B種の接地抵抗値の表のみ**（第17条第2項第一号）。A種 10Ω（第1項第一号）・C種 10Ω/500Ω（第3項第一号）・D種 100Ω/500Ω（第4項第一号）は表ではなく **条文本文** で規定される。下表はA〜D種を1表にまとめた学習用の再構成版。


| 接地工事の種類 | 接地抵抗値 | 緩和規定 |
|---|---|---|
| **==A種==接地工事** | ==**10**Ω以下== | （緩和規定なし） |
| **==B種==接地工事** | 変圧器の高圧側電路または特別高圧側電路の **1線地絡電流のアンペア数で ==150== を除した値** に等しいオーム数以下（==**150÷Ig Ω以下**==）＝17-1表「下記以外の場合」 | **高圧又は35,000V以下の特別高圧の電路と低圧電路を結合する変圧器** で、混触により低圧電路の対地電圧が150Vを超えた場合に自動的に高圧・特別高圧の電路を遮断する装置を設けるとき、遮断時間 **1秒を超え2秒以下** で ==**300÷Ig**== ／ **1秒以下** で ==**600÷Ig**==（17-1表） |
| **==C種==接地工事** | ==**10**Ω以下== | 低圧電路において **地絡を生じた場合に ==0.5秒以内==** に当該電路を自動的に遮断する装置（一般に漏電遮断器ELB）を施設するときは ==**500**Ω以下==（**電技解釈第17条第3項**） |
| **==D種==接地工事** | ==**100**Ω以下== | 低圧電路において **地絡を生じた場合に ==0.5秒以内==** に当該電路を自動的に遮断する装置（一般に漏電遮断器ELB）を施設するときは ==**500**Ω以下==（**電技解釈第17条第4項**） |

**穴埋めキーワード**: 10Ω以下／150÷Ig／300÷Ig／600÷Ig／100Ω以下／500Ω以下／0.5秒以内

#### 原典の 17-1表（B種接地工事の接地抵抗値）
...
（備考）Ig は、当該変圧器の高圧側又は特別高圧側の電路の1線地絡電流（単位: A）。Ig の値は ①実測値、②高圧電路では **17-2表に規定する計算式** により計算した値（計算結果は小数点以下切り上げ・**2A未満となる場合は2A**とする）、③特別高圧電路で実測が困難な場合は線路定数等により計算した値、のいずれかによる（第17条第2項第二号 イ・ロ・ハ）。

#### 原典の 17-2表（1線地絡電流 Ig の計算式）

同梱PDF p.32 を 2026-08-28 に照合（数式はテキスト抽出では復元できないため、該当領域を画像化して読み取った）。**2026-09-11 に独立再検証し、下表・備考・結合セルの読みすべてに相違なしを確認した**（下記「再検証のしかた」）。

??? note "再検証のしかた（テキスト抽出が壊れる理由と、機械で読み直す手順）"
    **テキスト抽出が落とすのはプライム記号だった。** `page.get_text()` で 17-2表 を読むと `L－100` `L－1` と出て **$L$ と $L'$ の区別が消える**。原因は、プライムが `SymbolMT` の**私用領域コードポイント `U+F0A2`** で置かれていることで、通常のテキスト抽出はこれを空白として捨てる。$V'$ も同様に $V$ に見える。**「テキスト抽出では数式が読めない」の正体はレイアウトだけではなく、この文字コードにもある。**

    そのため画像を見なくても、**文字単位の座標を取れば機械で復元できる**。

    ```python
    import pymupdf
    p = pymupdf.open("docs/assets/pdf/denken-kaishaku-r07-11.pdf")[31]   # PDF p.32
    for b in p.get_text("rawdict")["blocks"]:
        for l in b["lines"]:
            for s in l["spans"]:
                for ch in s["chars"]:
                    x, y = ch["origin"]
                    if x > 250 and 545 < y < 595:
                        c = "′" if ord(ch["c"]) == 0xF0A2 else ch["c"]
                        print(f"y={y:6.1f} x={x:6.1f} {c!r} {s['font']}")
    ```

    y でバンド化して x 順に並べると、分子・分数線・分母が分離して読める（第1行は `V′` → 線 → `3` の入れ子分数が `L－100` に掛かり、その全体が `150` の上に乗る形）。

    **結合セルも罫線座標で確定する。** 「中性点接地式電路」行が I₂ を共有する根拠は、`p.get_drawings()` の横罫線を並べると **計算式欄（x 256.9〜539.9）を横切る線が y=524.6／541.1／605.6／670.7／751.2 の5本しかない**こと。電路の種類欄には y=654.1 に線があるが（x 51.2〜256.4）、**計算式欄には無い**。したがって y=605.6〜670.7 の1セルが「電気ボイラー等」行と「中性点接地式電路」行の両方を覆う。

| 電路の種類 | 区分 | 計算式 |
|---|---|---|
| 中性点非接地式電路 | 下記以外のもの | $I_1 = 1 + \dfrac{\frac{V'}{3}L - 100}{150} + \dfrac{\frac{V'}{3}L' - 1}{2}$（第2項及び第3項の値は、それぞれ値が **負となる場合は 0** とする） |
| 中性点非接地式電路 | 大地から絶縁しないで使用する電気ボイラー、電気炉等を直接接続するもの | $\sqrt{I_1{}^2 + \dfrac{V^2}{3R^2} \times 10^6}$（**この式は下の「中性点接地式電路」行と結合セル**で共用） |
| 中性点接地式電路 | — | 同上 $\sqrt{I_1{}^2 + \dfrac{V^2}{3R^2} \times 10^6}$ |
...
| 記号 | 定義 | 単位 |
|---|---|---|
| $V'$ | 電路の公称電圧を **1.1** で除した電圧 | kV |
| $L$ | 同一母線に接続される高圧電路（電線に**ケーブルを使用するものを除く**）の **電線延長** | km |
| $L'$ | 同一母線に接続される高圧電路（電線に**ケーブルを使用するものに限る**）の **線路延長** | km |
| $V$ | 電路の公称電圧 | kV |
| $R$ | 中性点に使用する抵抗器又はリアクトルの電気抵抗値（**中性点の接地工事の接地抵抗値を含む**） | Ω |
| $X$ | 中性点に使用するリアクトルの誘導リアクタンスの値 | Ω |

!!! warning "17-2表の3つのひっかけ"
    1. **$L$ は「電線延長」・$L'$ は「線路延長」** — 語が違う。ケーブルを**除く**側が電線延長（$L$）、ケーブルに**限る**側が線路延長（$L'$）。
    2. **$V'$ は公称電圧そのものではない** — 公称電圧を **1.1で除した**電圧。一方 $V$（ボイラー等・リアクトル接地式の式で使う）は**公称電圧そのもの**。同じ表の中で $V$ と $V'$ の意味が違う。
    3. **「中性点接地式電路」の計算式欄は空欄に見えるが結合セル** — 原典の 17-2表 では、$\sqrt{I_1{}^2 + V^2/(3R^2)\times 10^6}$ のセルが「電気ボイラー等」行と「中性点接地式電路」行の**2行にまたがる結合セル**になっている（両行を仕切る罫線が計算式欄まで伸びていない）。したがって中性点接地式電路も**この式を使う**。式の意味も整合する — $V^2/(3R^2)\times10^6$ は相電圧 $V/\sqrt3$ [kV] を中性点抵抗 $R$ で割った抵抗分地絡電流の2乗、$I_1$ は静電容量分で、その**ベクトル和**が1線地絡電流になる。

    第2項・第3項が負なら 0 とする点、計算結果は小数点以下切り上げ・**2A未満なら2A**とする点も併せて押さえる。

### 接地線の最小太さ（A〜D種・学習用整理表）

!!! note "原典の「17-3表」はB種専用"
    原典で「17-3表」と呼ばれるのは **B種の接地線の表のみ**（第17条第2項第三号ロ）。A種 2.6mm/1.04kN は第1項第二号ロ、C種 1.6mm/0.39kN は第3項第二号ロ（D種は第4項第二号で第3項を準用）の **条文本文** で規定される。下表はA〜D種を1表にまとめた学習用の再構成版。
...
原典の 17-3表 は上記のとおり **移動使用・可とう性部分（キャブタイヤケーブル 断面積8mm²以上）の行を含む3区分**。学習用整理表ではこの行を省略している。
...
| 接地抵抗 | 「**10**Ω以下」 | 緩和規定なし（ELB緩和は C・D種のみ） |
...
| 抵抗値（原則） | **10**Ω以下 | **100**Ω以下 |
| 抵抗値（緩和） | 漏電遮断器 **0.5秒以内動作** → **500**Ω以下 | 漏電遮断器 **0.5秒以内動作** → **500**Ω以下 |
...
|---|---:|---|
| **10**Ω以下（A種規格内） | **≦ 100V** | ✅ 感電閾値（150V）以下に抑制 |
| 30Ω（規格超過） | 300V | ⚠️ 危険電位 |
| **100**Ω（規格大幅超過） | **1,000V** | ❌ 致命的高電位（即感電死リスク） |
...
**読み解き**: 高圧機器の絶縁劣化で漏電が発生すると、漏電電流は外箱→A種接地（**10**Ω以下）→接地線→接地極→大地という経路で流れる。同時にこの電流は **零相電流** としてZCTで検出され、GR（地絡継電器）が動作してVCB（高圧遮断器）を作動させ電源を切る。**A種10Ωが効く核心は「地絡電流経路の確保＝ZCT/GRの確実な動作担保」**。
...
- C種（300V超）: 機器電圧が高いため、A種と同じ **10**Ω に揃えて対地電圧を抑制
- D種（300V以下）: 機器電圧が低くリスクが低いため **100**Ω で許容
...
!!! warning "暗記補助：物理根拠ではなく公式根拠で覚える"
    「2.6mm」「4mm」の数値は **法令上の規定値** であり、物理的に「絶対にこの太さでなければならない」という導出があるわけではない。設計上の安全余裕を考慮した行政判断（電技解釈第17条第1項第二号〈A種〉・第2項第三号 17-3表〈B種〉・第3項第二号〈C種・D種準用〉）。試験では **数値そのもの** を覚えること。
...
| **C種** | ✅ | **500**Ω以下 | 漏電を生じた場合に **0.5秒以内** に自動遮断する装置を施設 |
| **D種** | ✅ | **500**Ω以下 | 漏電を生じた場合に **0.5秒以内** に自動遮断する装置を施設 |
...
|------|---------|---------|-----------|--------|
| **A種** | 高圧・特別高圧機器の外箱 | **10**Ω以下 | **2.6mm以上** | ❌ なし |
| **B種** | 高低圧結合変圧器の低圧側中性点 | **150÷Ig 以下** | **2.6mm or 4mm** | ❌（別ルート緩和） |
| **C種** | 300V超 低圧機器の外箱 | **10**Ω以下 | **1.6mm以上** | ✅ 500Ω以下 |
| **D種** | 300V以下 低圧機器の外箱 | **100**Ω以下 | **1.6mm以上** | ✅ 500Ω以下 |
...
| **C・D種引張強さ** | 0.39kN 以上 |
| **C・D種ELB緩和** | 漏電遮断器 **0.5秒以内** 動作 → **500**Ω以下（共通） |
...
| 出題テーマ | 関連条文 | 正答の根拠 |
|----------|---------|----------|
| A・B・C・D種の接地抵抗値 | **第17条** | B種は17-1表・A/C/D種は条文本文（第1・3・4項） |
| 接地線の最小太さ | **第17条** | B種は17-3表・A/C/D種は条文本文（第1・3・4項） |
| ELB緩和（C・D種500Ω） | **第17条** | 第17条第3項・第4項 |
| B種の自動遮断装置緩和（300/600÷Ig） | **第17条** | 17-1表（高圧又は35,000V以下の特高と低圧を結合する変圧器） |
...
| # | 切り口 | 典型的な問い方 | 正答の核心 |
|---|--------|---------------|----------|
| 1 | **種別判定** | 「対象機器の電圧から A〜D種のどれか」 | 各種の適用対象（条文本文）で電圧→種別 |
| 2 | **接地抵抗値（穴埋め）** | 「C種接地工事の接地抵抗値は」 | **10**Ω以下（ELB時 500Ω以下） |
| 3 | **接地線太さ（穴埋め）** | 「B種接地工事のうち高圧結合変圧器の場合の最小直径は」 | **2.6mm以上** |
| 4 | **計算（B種）** | 「Ig=4A、自動遮断装置1秒以内の場合のB種接地抵抗」 | **600÷4 = 150**Ω以下（1秒以下→600/Ig。1秒超2秒以下なら300/Ig=75Ω） |
| 5 | **緩和規定の適用判定** | 「漏電遮断器0.5秒以内動作時のD種接地抵抗値は」 | **500**Ω以下 |
...
| 🔴 致命的 | B種は常に4mm以上 | **17-3表の軟銅線太さは2区分**：高圧・第108条特高架空結合 2.6mm／その他 4mm |
...
??? note "解説と復習導線"
    ...
    「B種＝4mm一律」と覚えると誤答する。17-3表の軟銅線太さは2区分。
...
| 誤った記述 | 正しい記述 | なぜ違うか |
|---|---|---|
| 「A種の接地抵抗は100Ω以下」 | **10**Ω以下 | 100ΩはD種。A種は高圧なので1桁厳しい |
| 「A種の線径は4mm以上」 | **2.6mm以上** | 4mmはB種特別高圧結合。線径順は B(特高) > A=B(高圧) > C=D |
| 「B種は常に4mm以上」 | 高圧結合 **2.6mm** ／ 特別高圧結合 **4mm** | 17-3表の軟銅線太さは2区分 |
...
- 新規機器の電圧 → 各種接地工事の適用対象（条文本文）で種別決定
...
| 年次点検 | 接地抵抗測定記録 | 第17条の規定値（B種は17-1表）以下であることの担保 |
...
!!! note "H28問2 の帰属訂正（2026-06-11）"
    旧版で主軸に掲載していた H28問2（接地工事）は、[denken-ou houkih28-2](https://denken-ou.com/houkih28-2/) 一次照合で**第19条・第24条・第28条の複合出題**（空欄出典: (ア)(イ)(エ)＝第19条／(ウ)＝第28条）と確認されたため、[第19条](19.md) ほかへ再帰属した（kakomon.yml 同日訂正・PR #81 平成スポット監査の後続）。あわせて実績表の R07上問11 は R07下問11 の期誤記だったため是正（R07上問11 は水力計算・フッター注記と整合）。

### 出題パターン分析

- **B種計算**: 7回（最頻出パターン。Ig 与件 → 150÷Ig or 緩和式適用）
- **A〜D種総合穴埋め**: 4回（R07下／R06下／R04上／H30）
- **接地線太さの正誤**: 1回（H24問6）
- **特殊条件（埋設深さ等）**: 1回（R07下問11）
- **論説**: 1回（R01問6）
...
!!! note "予測"
    **B種計算問題は最頻出**（直近5年で5/8出題）→ 必須対策。
    ...
    - 隠し玉: **17-3表の軟銅線太さ2区分**（B種高圧結合 2.6mm vs 特高結合 4mm）の引っかけ
...
- [ ] A種接地抵抗値 = **10**Ω以下（緩和なし）／線径 **2.6mm以上**（引張強さ1.04kN）
- [ ] B種接地抵抗値 = **150÷Ig 以下**（緩和：1秒超〜2秒以下 300÷Ig／1秒以下 600÷Ig）
- [ ] B種線径 = 高圧結合 **2.6mm以上** ／ 特別高圧結合 **4mm以上**（引張強さ2.46kN）
- [ ] C種接地抵抗値 = **10**Ω以下（ELB0.5秒以内 → **500**Ω以下）／線径 **1.6mm以上**
- [ ] D種接地抵抗値 = **100**Ω以下（ELB0.5秒以内 → **500**Ω以下）／線径 **1.6mm以上**
...
!!! note "条文原文 — 電技解釈 第17条（令和7年11月版・逐語）"
    ...
    >
    > **第2項** B種接地工事は、次の各号によること。
    >
    > 　一 接地抵抗値は、17-1表に規定する値以下であること。
    >
    > ※ 17-1表 は本ページの解説表を参照（原文の表は未転記）
    >
    ...
    > 　　ロ 17-3表に規定するものであること。
    >
    > ※ 17-3表 は本ページの解説表を参照（原文の表は未転記）
    >
    > 　四 第1項第三号及び第四号に準じて施設すること。
    >
    > **第3項** C種接地工事は、次の各号によること。
    >
    ...
    >
    > **第6項** D種接地工事を施す金属体と大地との間の電気抵抗値が100Ω以下である場合は、D種接地工事を施したものとみなす。

---

## 監修ログ

- **2026-08-31 17-2表の結合セル誤読を是正**（Fable 反証監査 → Claude が原典PDFで再照合）: 旧記述「中性点接地式電路は原典でも計算式欄が空欄」は**誤り**。同梱PDF p.32 の罫線を PyMuPDF で実測したところ、「電気ボイラー等」行と「中性点接地式電路」行を仕切る罫線（y=654.1）は x≤256.4 の見出し列にしか存在せず、計算式列（x=256.9〜539.9）に達していない。上下の罫線（y=605.6・y=670.7）は計算式列を横切っている。該当領域を画像化して目視でも確認済み。したがって $\sqrt{I_1{}^2+V^2/(3R^2)\times10^6}$ は**2行にまたがる結合セル**で、中性点接地式電路も同式を使う。式の意味とも整合する（$V^2/(3R^2)\times10^6$＝相電圧 $V/\sqrt3$ を中性点抵抗 $R$ で割った抵抗分地絡電流の2乗、$I_1$＝静電容量分、そのベクトル和）。これに伴い「17-2表の3つのひっかけ」#3 を差し替えた。**結合セルの向きはセルの bbox やテキスト抽出では決まらず、罫線を読むか画像化するしかない**という教訓。
...
- **2026-08-28 出題実績を SoT と整合**（Claude・kakomon.yml 突合）: メタを 14回→**15回** に是正（SoT 集計・本文2箇所）。
- **2026-08-28 17-2表の honest-hold 解消**（Claude・PDF 画像化で読み取り）: 前版で「機械抽出では数式が復元できない」として記載を見送っていた **17-2表（1線地絡電流 Ig の計算式）**を、同梱PDF p.32 の該当領域を高解像度で画像化して読み取り、全4行＋備考6記号を収載した。確定した内容: ①中性点非接地式（下記以外）＝ 1＋((V'/3)L−100)/150＋((V'/3)L'−1)/2、第2項・第3項が負なら0 ②電気ボイラー等＝√(I₁²＋V²/(3R²)×10⁶) ③中性点接地式電路（**2026-08-31 訂正**。旧記述「原典でも計算式欄が空欄」は誤り。罫線を実測したところ、両行を仕切る罫線が計算式欄に達しておらず、②の式が「電気ボイラー等」行と結合セルで共用されていた） ④中性点リアクトル接地式＝√(((V/√3)R/(R²+X²)×10³)²＋(I₁−(V/√3)X/(R²+X²)×10³)²)。備考では **L＝電線延長（ケーブルを除く）／L'＝線路延長（ケーブルに限る）** と語が異なる点、**V' は公称電圧を1.1で除した電圧**で同表内の V（公称電圧そのもの）と意味が違う点を明示。典拠: 同梱PDF p.32-33。
- **2026-08-28 表の原典突合**（Fable 一任・座標ベース復元表と照合）: ①「17-1表＝A〜D種の接地抵抗値」「17-3表＝A〜D種の接地線太さ」という表名の誤帰属を全面是正（原典の17-1表・17-3表はいずれもB種専用。A・C・D種の数値は条文本文＝第1・3・4項で規定）。②B種緩和（300/600÷Ig）の適用対象を原典どおり「高圧又は35,000V以下の特別高圧の電路と低圧電路を結合する変圧器」に是正（旧記述「特別高圧結合は別途規定」は誤り）。③原典の17-1表（3行）・17-3表（キャブタイヤ8mm²行を含む3行）をセクション2に原文どおり収載。④17-2表（Ig計算式）は機械抽出で数式が復元できないため式を記載せず、同梱PDF p.32 の目視照合が必要である旨を明記。⑤2.6mm/4mm の根拠条文誤記（第6項→第1項第二号・第2項第三号）を訂正。典拠は「同梱PDF p.32-33 の 17-1表・17-2表・17-3表」。
- **2026-08-28 監査是正**（ChatGPT/Fable 監査 → Claude 一次照合）: 想定問題4の解答を是正（Ig=4A・1秒以内→300÷4=75Ω→**600÷4=150**Ω）。17-1表では 1秒以下＝600/Ig、1秒超2秒以下＝300/Ig。同ページ内の他ブロックは正しく150Ωとしており内部矛盾していた。
...
- **2026-05-08 v3.0 全面再構築（条文番号誤記訂正）**: タイトル「A種接地工事」→ **「接地工事の種類及び施設方法」** に昇格。A〜D種統合構造で v2.4.3 完全準拠の再構築。
    ...
    - **未確認領域**: 電技解釈の **逐語原文** は本記事では取得できておらず、JEEA講座および接地規程解説からの **要旨** を記載している。試験対策上は要旨で十分だが、行政手続等で逐語原文が必要な場合は経産省の解釈原本を別途参照されたい。
...
*最終確認: 2026-05-08 | ステータス: v3.1（ChatGPT校閲反映：B種緩和訂正・C/D使用電圧判定・A種10Ω純化・B種太さ3区分・一枚暗記表） ｜ 条文番号: ✅ 一次ソース照合済（JEEA／経産省告示PDF／サンコーシヤ） ｜ 出題実績: ✅ kakomon DB 14件照合済（直近 R07下問4・R07下問11／B種計算最頻出。旧版「R07上問11」は誤記＝R07上問11は水力計算で接地ではない・2026-06-10 denken-ou 照合で訂正。H28問2 は houkih28-2 一次照合で第19条・第24条・第28条の複合出題と確認し再帰属・2026-06-11） ｜ 数値根拠: ✅ 公式根拠記載済（17-1表 抵抗値／17-3表 線太さ3区分／A種10Ω=外箱電位抑制+GR動作／150÷Ig 混触防止式） ｜ [バージョニング基準](../../reference/versioning.md) ｜ [テンプレv2.4仕様](../../reference/template-v24.md)*
