...
（備考）Ig は、当該変圧器の高圧側又は特別高圧側の電路の1線地絡電流（単位: A）。Ig の値は ①実測値、②高圧電路では **17-2表に規定する計算式** により計算した値（計算結果は小数点以下切り上げ・2A未満となる場合は2Aとする）、③特別高圧電路で実測が困難な場合は線路定数等により計算した値、のいずれかによる（第17条第2項第二号）。**17-2表の計算式そのものは本記事では記載しない**——PDFからの機械抽出では数式セルが復元できないため、式の確認には同梱PDF p.32 の 17-2表 の目視照合が必要。
