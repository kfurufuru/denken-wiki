...
!!! warning "17-2表の3つのひっかけ"
    ...
    3. **中性点接地式電路には計算式が無い** — 17-2表 に行はあるが計算式欄は空欄。「17-2表で全部計算できる」ではない。
...
- **2026-08-28 17-2表の honest-hold 解消**（Claude・PDF 画像化で読み取り）: 前版で「機械抽出では数式が復元できない」として記載を見送っていた **17-2表（1線地絡電流 Ig の計算式）**を、同梱PDF p.32 の該当領域を高解像度で画像化して読み取り、全4行＋備考6記号を収載した。確定した内容: ①中性点非接地式（下記以外）＝ 1＋((V'/3)L−100)/150＋((V'/3)L'−1)/2、第2項・第3項が負なら0 ②電気ボイラー等＝√(I₁²＋V²/(3R²)×10⁶) ③**中性点接地式電路は原典でも計算式欄が空欄** ④中性点リアクトル接地式＝√(((V/√3)R/(R²+X²)×10³)²＋(I₁−(V/√3)X/(R²+X²)×10³)²)。備考では **L＝電線延長（ケーブルを除く）／L'＝線路延長（ケーブルに限る）** と語が異なる点、**V' は公称電圧を1.1で除した電圧**で同表内の V（公称電圧そのもの）と意味が違う点を明示。典拠: 同梱PDF p.32-33。
