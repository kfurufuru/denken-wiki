...
!!! note "📊 試験対策メタ"
    **出題頻度**: 🔥🔥🔥🔥🔥（過去14年で 5 回出題・**R05上以降4期連続**で法規分野の最頻出）

    **重要度**: S（最重要・R05以降毎回出題されており **次回以降も最優先で警戒すべき論点**）

    **直近出題**: R07下 問2（穴埋）／R07上 問2（論説）／R06上 問2（論説）／R05上 問2（穴埋）／R02 問2（穴埋）

    （旧版は H26問2 も第3条実績に数え「6回」としていたが、denken-ou houkih26-2 照合で H26問2 は**報告規則第5条**（設置者の出力・電圧変更／廃止の報告）と判明し除外＝第3条の実出題は5回。2026-06-10）
...
| **照合日** | 2026-05-10（eGov 一次ソースで第3条 第1項表（14号）と第2項本文を全件再照合・kakomon.yml で出題実績5件照合済〔報告規則第3条明示〕。2026-06-10 に H26問2 を第5条と確定し 6→5 に訂正） |
...
???+ note "📜 条文原文 — eGov 完全版（黄色マーカー=過去問で穴埋め出題された語・クリックで開閉）"
    ...
    **凡例**: <mark>黄色マーカー</mark> = 過去14年で **実際に穴埋め出題された** キーワード（R02問2／R05上問2／R06上問2／R07上問2／R07下問2 で照合。H26問2 は報告規則第5条のため除外）。**bold** は条文の主要語句（試験頻出だが穴埋め実績は限定的なものも含む）。
    ...
    **黄色マーカー対象の出題実績根拠**:
    ...
    | マーカー語 | 出題形式 | 該当年度 |
    |---|---|---|
    | 事故 | 穴埋（報告事象として） | R02問2／R05上問2／R06上問2／R07上問2／R07下問2 |
    | 報告先 | 穴埋（報告先語句） | R07下問2／R05上問2 等 |
    | 入院 | 穴埋 | **R05上問2 (エ)** — 「に入院」が空欄 |
    | 産業保安監督部長 | 穴埋（所轄〜長） | R02問2／R05上問2 等 |
...
### 実問題: R05上 問2（第1条第2項の用語定義 × 第3条の事故報告）
...
??? success "解答"
    - **ア = せん絡**（[第1条第2項 四号「電気火災事故」](jiko-1.md) 由来）
    - **イ = 直ちに**（[第1条第2項 五号「破損事故」](jiko-1.md) 由来）
    - **ウ = 操作**（[第1条第2項 七号「供給支障事故」](jiko-1.md) 由来）
    - **エ = に入院**（**本条 第3条第1項表 第1号** 感電死傷の報告閾値）

??? note "解説と復習導線"
    !!! tip "この問題の構造的意義"
        本問は **第1条第2項（用語定義）と第3条（報告手続）を一体で問う** 法規分野の典型出題。3つの空欄が [第1条](jiko-1.md) の用語定義に直結し、1つ（エ）が本条 **第3条第1項表 第1号** の報告対象閾値。**第3条の暗記だけでは不十分** で、第1条第2項の各号定義を併せて押さえる必要がある。

    **本条由来の (エ) に入院** — 解説

    - **条文文言**: 第3条第1項表 **第1号** =「感電により人が死亡し、又は病院若しくは診療所**に入院**した場合」が報告対象
    - **誤答パターン**: 「で治療」「で外来診療」「に通院」「で救急搬送」 → 条文は **「に入院」** で固定。外来診療のみ・通院のみ・救急搬送のみは対象外（**入院加療**が客観閾値）
    - **混同注意**: 「**死亡 又は 入院**」が条文。軽傷・外来診療のみは報告対象外。「全件報告」と誤覚えしない（→ 本記事 セクション8 頻出ひっかけ・落とし穴 参照）
    - **なぜ「入院」閾値か** = 重大事故への絞り込み＋医療機関判断による **客観的な区分** が可能だから

    **(ア)(イ)(ウ) は [第1条](jiko-1.md) 由来** — 要点のみ

    - (ア) **せん絡** = 電気火災事故の電気的要因の例示（漏電・短絡・**せん絡**の3並列）
    - (イ) **直ちに** = 破損事故の機能低下／喪失の **即時性** を表す副詞
    - (ウ) **操作** = 供給支障事故の原因類型（「破損事故 又は 誤**操作**」で対句構造）
    - 詳しい解説は [jiko-1.md セクション6 過去問チャレンジ](jiko-1.md) を参照

    !!! warning "出題パターンの教訓"
        R05上 問2 は **(エ) のみ第3条由来・(ア)(イ)(ウ) は第1条由来** という複合形式。「第1条は出題されない」と思い込んで対策を怠ると、4空欄中3つを取りこぼす。**第1条第2項の用語定義と第3条の報告対象事故をセットで覚える**こと。
...
!!! tip "学習ラダー方式（Why→What→How）"
    暗記前に **「なぜ二段階報告か」（Step1: 理解）** をつかむ → **速報・詳報の条文穴埋めで構造確認（Step2）** → **「報告／届出／承認」の射程区別（Step3: 応用）** へ段階接続。Step1で「監督庁側の運用」を理解できていれば、Step2の「時／日」「24／30」「電話／様式第13」の対称性が腑に落ちる。

!!! abstract "問題1【Step1: 理解】なぜ速報・詳報の二段階か"
    本条は事故発生時に「速報（24時間以内・電話等）」と「詳報（30日以内・様式第13）」の **二段階報告** を求めている。なぜ一段階（例: 30日以内・様式第13のみ）ではなく、わざわざ二段階に分けているのか、**監督庁側の運用視点** から理由を1つ挙げよ。

??? success "解答 1"
    監督庁が **初動対応と原因分析を分けて処理する** ため。

    **なぜ重要か**:

    - 速報の目的 = **初動対応**（同種事故の連鎖防止・現場の安全確保のため監督庁が即時把握する必要）
    - 詳報の目的 = **原因分析と再発防止**（事業者側の調査結果を踏まえた構造的な対策立案）
    - 監督庁は速報を受けた直後に「同種設備への注意喚起」を発出することがある。これは詳報を待っていたら間に合わない。一方、詳報は原因究明後の構造データが必要なので30日の猶予が現実的。
    - つまり「速報24時間／詳報30日」を語呂で覚えるのではなく、**「監督庁が何をするのか」を理解** すれば数値は自然に思い出せる。

!!! abstract "問題2【Step2: 構造確認】速報・詳報の条文穴埋め"
    自家用電気工作物を設置する者は、その工作物に関して事故が発生したときは、当該事故の発生を（　ア　）から（　イ　）時間以内可能な限り速やかに（　ウ　）等の方法により所轄産業保安監督部長に報告し（**速報**）、さらに当該事故の発生を（　エ　）から起算して（　オ　）日以内に（　カ　）の報告書を提出しなければならない（**詳報**）。

??? success "解答 2"
    - ア = **知った時** ／ イ = **24** ／ ウ = **電話**（**速報**）
    - エ = **知った日** ／ オ = **30** ／ カ = **様式第13**（**詳報**）

    **構造**: ［速報＝即時・口頭系・"時"単位］と［詳報＝期限内・文書系・"日"単位］の対称対比。

    Step1で「初動対応 vs 原因分析」を理解していれば、速報は「電話／時間」（即時性重視）・詳報は「様式／日」（精度重視）と論理導出できる。「発生した時」「48時間」「文書」（速報側）／「発生した日」「90日」「自由書式」（詳報側）は典型的な誤答選択肢。**ア・エで「時」と「日」が違う** ことに気付けるかが決定的。
...
!!! abstract "問題4【Step3: 応用】「報告・届出・承認」の射程区別"
    電気事業法および関係省令上、次の事象に対応する **行政手続** として正しい組合せはどれか。

    - A. 自家用電気工作物で感電死亡事故が発生した
    - B. 自家用電気工作物の設置工事（工事計画対象）を着工する
    - C. 主任技術者を選任した
    - D. 保安規程を変更した
...
??? success "解答 4"
    ...
    **理由（射程区別）**:
    ...
    | 手続 | 性格・タイミング | 根拠 |
    |------|----------------|------|
    | **報告** | 事後・事実通知（許可不要） | 事故 = 電気関係報告規則第3条（**本条**） |
    | **届出** | 事前・事実の届出（認可・許可・承認は不要）。**条文の語は「届け出」であって「通知」ではない** | 工事計画 = 電気事業法第48条／主任技術者選任 = 同第43条第3項／保安規程変更 = 同第42条第2項 |
    | **承認** | 行政庁の事前判断必要（許可と類似） | 工事計画変更命令対応等（限定的） |

    - **報告 vs 届出** の最大の違いは **タイミング**（事後＝報告／事前＝届出）。事故は事後だから「報告」、工事は事前だから「届出」。
    - 「事故も届出」「主任技術者選任も承認」と誤読する罠が頻出。**同じ法律内で手続名が違えば射程が違う**。
    - 本条（電気関係報告規則）は **「報告」専用の規則**。届出・承認は別の規則・条文系統で扱う（→ [報告先・届出先 早見マップ](../../reference/reporting-destinations.md) で横断確認可）。

!!! abstract "問題（補足）: 報告先と上位法"
...
??? success "解答（補足）"
    ...
    「電気設備技術基準」「経済産業大臣（直接）」は誤答。報告規則の基づく法律と権限委任先を区別する（旧問題4を補足問題として温存）。

---

### 該当 / 非該当 判定演習 — 過去問の事故報告該当判定（5択論説）

電気関係報告規則 第3条 第1項表 第1〜14号 の **要件に1つでも当てはまれば報告対象**、すべて当てはまらなければ報告対象外。同じ「**事故報告に該当しないものはどれか**」形式が **H22 問3／R06上 問2／R07上 問2** で繰り返し出題されている（**3問とも答えは(4)・「自社設備のみを誤操作で損壊」が非該当の定型形**）。

!!! tip "判定の4ステップ — どの号にも当たらなければ非該当"
    1. **人**：死亡・入院があるか → 第1号（感電 **又は誤操作** による死傷）
    2. **火**：電気火災で **半焼以上** か → 第2号
    3. **他**：他物件への損傷／系統への波及はあるか → 第3号・第10〜12号
    4. **規模・社会**：主要工作物の破損（第4号ヌ＝**10,000V以上の需要設備**）／大規模供給支障／ダム異常放流／社会的影響 → 第4〜9号・第13・14号

    4ステップすべてに当てはまらないときのみ **報告対象外**。

#### 演習1 — H22 問3（受電電圧6,600V 自家用設置者）

!!! abstract "問題（平成22年 法規 問3）"
    「電気関係報告規則」に基づく、事故報告に関して、受電電圧6,600 V の自家用電気工作物を設置する事業場における下記(1)〜(5)の事故事例のうち、**事故報告に該当しないもの** はどれか。

    (1) 自家用電気工作物の破損事故に伴う構内1号柱の倒壊により道路をふさぎ、長時間の交通障害を起こした。

    (2) 保修作業員が、作業中誤って分電盤内の低圧200 V の端子に触れて感電負傷し、治療のため3日間入院した。

    (3) 電圧100 V の屋内配線の漏電により火災が発生し、建屋が全焼した。
...
??? note "解説 — 条文要件 vs 本ケースの照合"
    ...
    |---|---|---|---|
    | **(1)** 1号柱倒壊・長時間交通障害 | **第14号**：第1〜13号 以外の事故であって、電気工作物に係る **社会的に影響を及ぼした事故** | 構内柱の倒壊で **道路を塞ぎ長時間の交通障害** が発生 → 第三者・地域社会に明白な影響あり、第14号の要件を満たす | ✅ **該当**（第14号） |
    | **(2)** 200V 端子で感電・3日入院 | **第1号**：感電等により人が **死亡** 又は **病院若しくは診療所に入院** した事故（電圧の高低・低圧/高圧は要件にない） | 治療のため **3日間入院** → 第1号の「入院」要件を満たす。電圧200Vは条文の閾値とは無関係 | ✅ **該当**（第1号） |
    | **(3)** 100V 漏電による建屋 **全焼** | **第2号**：漏電・短絡・せん絡 その他の電気的要因による火災で、**工作物の場合は半焼以上に限る** | 「漏電」＝電気的要因／「全焼」は **半焼以上を当然満たす**。電圧100Vは条文の閾値とは無関係 | ✅ **該当**（第2号） |
    | **(4)** 誤操作で **高圧の誘導電動機を損壊** させた | **第3号**は「電気工作物の破損・誤操作等により **他の物件** に損傷を与え、又はその機能の全部・一部を損なわせた事故」を要件とする／**第4号ヌ**は「電圧 **10,000V以上** の需要設備」の主要電気工作物破損事故を要件とする | 損壊したのは **自社の誘導電動機（自分の電気工作物）** のみで「他の物件」ではない／受電電圧 **6,600V<10,000V** で第4号ヌの規模要件にも当たらない／感電・火災・供給支障・社会的影響もない → **第1〜14号 のどれにも当たらない** | ❌ **非該当**（=正解） |
    | **(5)** 落雷で高圧負荷開閉器破損・電気事業者へ供給支障 | **第12号**：一般送配電・配電・特定送配電事業者と接続された **電圧3,000V以上の自家用電気工作物** の破損・誤操作等により当該事業者に **供給支障** を発生させた事故 | 受電 **6,600V（≥3,000V）** の自家用工作物の破損 → **電気事業者（一般送配電事業者）に供給支障**。条文要件すべて充足。原因が落雷（自然現象）でも判定は結果で行うため **速報24h は必須**（第3条第2項ただし書で詳報のみ免除） | ✅ **該当**（第12号） |

    !!! warning "(4) を「電動機の損壊だから第3号に該当」と誤判定しない"
        条文は「**他の物件** に損傷を与え、又はその機能の全部・一部を損なわせた事故」。**自分の電気工作物（=損壊した電動機そのもの）が壊れただけ** では「他の物件」要件を満たさない。「他の物件」＝破損した電気工作物 **以外** の物件。

    !!! warning "(5) を「落雷＝自然現象だから対象外」と誤判定しない"
        判定は **原因（自然・人為）ではなく結果（条文要件）** で行う（→ セクション5 判定フロー）。落雷が原因でも結果が第12号 に該当する以上 **速報24h は義務**。詳報のみ第3条第2項ただし書（第4号ハ・第8〜13号 の自然現象由来）で免除。

---

#### 演習2 — R06上 問2（H22 問3 の変形版・受電電圧6,600V 自家用設置者）

!!! abstract "問題（令和6年上期 法規 問2）"
    「電気関係報告規則」に基づく、事故報告に関して、受電電圧6,600 V の自家用電気工作物を設置する事業場における下記(1)〜(5)の事故事例のうち、**事故報告に該当しないもの** はどれか。

    (1) 自家用電気工作物の破損事故に伴う構内柱の倒壊により、第三者の家屋に損傷を与えた。

    (2) 保修作業員が、作業中誤って分電盤内の低圧200 V の端子に触れて感電負傷し、病院に入院した。

    (3) 電圧100 V の屋内配線の漏電により火災が発生し、建屋が **半焼** した。

    (4) 従業員が、操作を誤って高圧の誘導電動機を損壊させた。

    (5) 落雷により高圧負荷開閉器が破損し、**一般送配電事業者** に供給支障を発生させたが、電気火災は発生せず、また、感電死傷者は出なかった。

??? success "解答：(4)"
    **該当しないもの = (4)**（H22 問3 と同じ）

??? note "解説 — H22との差分 + 各事例の判定"

    **H22 問3 との差分は3点**（いずれも該当方向の根拠が変わるのみ・正解は同じ(4)）：

    | 事例 | H22 問3 | R06上 問2 | 該当条文の変更 |
    |---|---|---|---|
    | (1) | 道路ふさぎ・長時間交通障害 | **第三者の家屋に損傷** | **第14号 社会的影響 → 第3号 他物件損傷** |
    | (3) | 建屋 **全焼** | 建屋 **半焼** | いずれも第2号「半焼以上」を充足（半焼はちょうど閾値） |
    | (5) | 「電気事業者」表記 | 「**一般送配電事業者**」表記 | 第12号（条文文言は「一般送配電事業者等」）— 実質同じ |

    **条文要件 vs 本ケース**:

    | 事例 | 該当条文の要件（条文は○○） | 本ケースの状況（今回のケースは△△） | 判定 |
    |---|---|---|---|
    | (1) 第三者家屋に損傷 | **第3号**：電気工作物の破損等により **他の物件に損傷** を与えた事故 | 構内柱倒壊で **第三者の家屋に損傷** → 第三者家屋は「他の物件」に該当 | ✅ **該当**（第3号） |
    | (2) 200V 感電・入院 | **第1号**：感電による **入院** | 入院 → 要件充足 | ✅ **該当**（第1号） |
    | (3) 100V 漏電・建屋 **半焼** | **第2号**：電気火災・**半焼以上** | 「半焼」はちょうど閾値で **半焼以上を充足** | ✅ **該当**（第2号） |
    | (4) 高圧誘導電動機を誤操作で損壊 | 第3号（他物件損傷）／第4号ヌ（10,000V以上需要設備の主要工作物破損） | 自社電動機の損壊のみ・他物件損傷なし／受電 6,600V<10,000V で第4号ヌも非該当 | ❌ **非該当**（=正解） |
    | (5) 落雷で高圧負荷開閉器破損→一般送配電事業者へ供給支障 | **第12号**：3,000V以上の自家用 → 一般送配電事業者へ供給支障 | 6,600V自家用 → 一般送配電事業者へ供給支障。要件充足。**落雷でも速報24h は必須** | ✅ **該当**（第12号） |

    !!! tip "「半焼」と「全焼」はいずれも第2号に該当"
        第2号は「**半焼以上**」が閾値。**半焼・全焼のどちらも該当**。「全焼でないと対象外」「半焼程度では軽微」と誤判定しないこと。

---

#### 演習3 — R07上 問2（機器構成バリエーション・受電電圧6.6kV 自家用設置者）

!!! abstract "問題（令和7年上期 法規 問2 — 要点）"
    受電電圧 6.6kV の自家用電気工作物を設置する事業場における次の事故事例のうち、電気関係報告規則に基づき設置者が所轄産業保安監督部長に報告すべき事故に **該当しないもの** はどれか。

    （正確な原文は出題機関公表の問題冊子を参照。本記事は試験対策のため5事例の要点を抽出して掲載）

    | 事例 | 概要 |
    |---|---|
    | (1) | 屋内配線の電気火災により **建屋が半焼** した |
    | (2) | 落雷により高圧負荷開閉器が焼損し、**一般送配電事業者に供給支障** を生じた |
    | (3) | 高圧断路器の **誤操作によりアーク熱で作業員が火傷** を負い **入院** した |
    | (4) | 高圧真空遮断器の **誤操作によりこれを損傷** し、操作不能となった |
    | (5) | **構内柱の折損により隣接建物に損傷** を与えた |

??? success "解答：(4)"
    **該当しないもの = (4)**（H22 問3／R06上 問2 と同じく自社設備のみ損壊パターン）

??? note "解説 — 条文要件 vs 本ケース"

    | 事例 | 該当条文の要件（条文は○○） | 本ケースの状況（今回のケースは△△） | 判定 |
    |---|---|---|---|
    | (1) 屋内配線の火災・建屋半焼 | **第2号**：電気的要因の火災・**工作物は半焼以上** | 屋内配線の電気火災で **建屋半焼** → 半焼以上を充足 | ✅ **該当**（第2号） |
    | (2) 落雷で高圧負荷開閉器焼損・一般送配電事業者へ供給支障 | **第12号**：3,000V以上の自家用 → 一般送配電事業者へ供給支障 | 高圧（≥3,000V）の負荷開閉器破損で供給支障 → 要件充足。**落雷でも速報必須**（第3条第2項ただし書は詳報のみ免除） | ✅ **該当**（第12号） |
    | (3) 高圧断路器誤操作・アーク熱で作業員入院 | **第1号**：感電 **又は電気工作物の破損若しくは誤操作若しくは操作しないこと** により人が **死亡又は入院** した事故 | 誤操作によるアーク熱火傷で **入院** → 第1号は「感電」だけでなく「電気工作物の誤操作による死傷」も対象。要件充足 | ✅ **該当**（第1号） |
    | (4) 高圧真空遮断器の誤操作で自社遮断器を損傷・操作不能 | 第3号（他物件損傷）／第4号ヌ（10,000V以上需要設備の主要工作物破損） | **自社の高圧真空遮断器を損壊させ操作不能になっただけ**。他の物件に損傷なし・感電・火災・供給支障なし／受電6.6kV<10,000V で第4号ヌも非該当 → **第1〜14号 のどれにも当たらない** | ❌ **非該当**（=正解） |
    | (5) 構内柱折損・隣接建物に損傷 | **第3号**：電気工作物の破損等により **他の物件に損傷** | 構内柱折損で **隣接建物に損傷** → 他の物件に該当 | ✅ **該当**（第3号） |

    !!! tip "第1号は「感電」だけでなく「誤操作による死傷」も含む"
        第1号 の条文は「**感電又は電気工作物の破損若しくは電気工作物の誤操作若しくは電気工作物を操作しないことにより** 人が死傷した事故」と広い。**アーク熱火傷・閃絡熱傷** も「電気工作物の誤操作・破損」由来であれば対象に含む。「直接の通電感電でないと対象外」は誤り。

    !!! warning "「機器を損壊した」だけでは非該当という定型パターン"
        H22 問3・R06上 問2・R07上 問2 **3問とも** 選択肢(4)＝「**自社の機器（電動機・遮断器）を誤操作で損壊させただけ**」が **非該当**。「**自社設備のみの損壊＋他物件損傷・人身・供給支障なし**」は第3条のどの号にも当たらない、というのが受験者に最も問われている構造。
...
| 観点 | 共通する出題意図 |
|---|---|
| **正解** | 3問とも **(4)** ＝「**自社の電気工作物を誤操作で損壊させただけ** の事例」 |
| **非該当の3根拠** | (a)他の物件に損傷なし → **第3号 非該当** ／(b)受電 6,600V<10,000V → **第4号ヌ 非該当** ／(c)感電・火災・供給支障・社会的影響なし → **第1・2・8〜12・14号 非該当** |
| **(1)(2)(3)(5)の該当根拠** | 必ず **「社会的影響／他物件損傷／入院／半焼以上／供給支障」** のいずれかに紐づく → 条文要件と機械的にマッチング |
| **時短のコツ** | (4)の構造「**自社設備のみの損壊で14号要件に1つも当たらない**」を見抜けば、(1)(2)(3)(5)の細部を読まなくても正解できる |
...
| 過去14年で5回出題（R02/R05上/R06上/R07上/R07下） | kakomon.yml 照合（報告規則第3条明示5件） | ✅ PASS |
...
| R07上 | 問2 | 論説（**該当判定5択**） | 自家用電気工作物の事故報告（受電6.6kV・5事例から非該当を選ぶ）— [演習3](#_5)で詳細解説 |
| R06上 | 問2 | 論説（**該当判定5択**） | 自家用電気工作物の事故報告（受電6,600V・5事例から非該当を選ぶ）— [演習2](#_5)で詳細解説 |
| R05上 | 問2 | 穴埋 | 事故の定義及び事故報告（電気関係報告規則）— **(ア)せん絡／(イ)直ちに／(ウ)操作 は[第1条第2項](jiko-1.md)由来**・(エ)に入院 は本条由来の複合出題 |
| R02 | 問2 | 穴埋 | 電気関係報告規則に基づく事故報告 |
| H26 | 問2 | （第5条） | ※自家用電気工作物設置者の**出力・電圧変更／廃止の報告**＝報告規則**第5条**。第3条（事故報告）とは別論点のため本条実績から除外（2026-06-10 denken-ou houkih26-2 照合） |
| H22 | 問3 | 論説（**該当判定5択**）※kakomon DB対象外（H23-R07） | 自家用電気工作物の事故報告（受電6,600V・5事例から非該当を選ぶ）— [演習1](#_5)で詳細解説。R06上問2／R07上問2 の原型 |

!!! note "出題傾向"
    過去14年で5回出題。**R05上以降4期連続出題**で頻度が急上昇している。穴埋・論説の双方で出題され、論説では「報告制度の趣旨」「速報と詳報の役割分担」「再発防止策」が問われる。穴埋では期限の数値（24h／30日）と起算点（知った時／知った日）が頻出。
...
- **2026-05-17 v2.11 R05上 問2 実問題ブロック追加**: ユーザー指摘「R05上問2の解説がない」への対応。セクション9「穴埋め過去問チャレンジ」冒頭に **R05上 問2 の実問題セット** を追加（問題文4空欄／解答／解説）。(エ)に入院 を本条第3条第1項表第1号由来として詳細解説（誤答パターン・混同注意・閾値設定の意義）、(ア)(イ)(ウ) は第1条由来として要点のみ記載し [jiko-1.md セクション6](jiko-1.md) へリンク誘導。jiko-1.md と同パターンで第1条＋第3条の複合形式を学習導線で接続。既存4問は「補強問題（汎用）」として残置・本文無改変。wiki_check.py PASS
- **2026-05-19 v2.12 該当/非該当 判定演習 3問追加**: ユーザー指摘「電気関係報告規則として該当するもの、しないものを追加して理解度を深めたい」+ 「過去問を確認して同様の該当/非該当の問題があればすべて事例追加」への対応。セクション9 末尾（補強問題4と セクション10 の間）に **該当/非該当 判定演習** 新サブセクション追加。**denken-ou.com** 一次照合で 同形式（5択論説・事故報告に該当しないものはどれか）の過去問を3問特定：
    - **演習1 = H22 問3**（ユーザ提示問題・受電6,600V・5事例：構内1号柱倒壊で交通障害／200V感電3日入院／100V漏電全焼／高圧誘導電動機誤操作損壊／落雷で負荷開閉器破損供給支障）→ 答え **(4)**
    - **演習2 = R06上 問2**（H22の変形版・受電6,600V・5事例：構内柱倒壊で第三者家屋損傷／200V感電入院／100V漏電半焼／高圧誘導電動機誤操作損壊／落雷で負荷開閉器破損→一般送配電事業者へ供給支障）→ 答え **(4)**
    - **演習3 = R07上 問2**（機器構成バリエーション・受電6.6kV・5事例：屋内配線火災で建屋半焼／落雷で高圧負荷開閉器焼損供給支障／高圧断路器誤操作でアーク熱火傷入院／高圧真空遮断器誤操作で損傷操作不能／構内柱折損で隣接建物損傷）→ 答え **(4)**
    - **共通構造**: 3問とも (4)＝「**自社設備を誤操作で損壊させただけ・他物件損傷/感電/火災/供給支障なし**」が非該当。受電6,600V<10,000V で第4号ヌ も非該当。3問の正解一致と共通構造を末尾の比較表で明示
    - **決め手の記述方針**: ユーザー指摘「条文は○○で今回のケースは△△なので該当する/該当しないと理由がわかるように」を反映。各事例で「該当条文の要件（条文は○○）」「本ケースの状況（今回のケースは△△）」の2列に分けて判定根拠を明示
    - **判定の4ステップ tip**: 人（第1号）／火（第2号半焼以上）／他（第3・10〜12号）／規模・社会（第4〜9号・第13・14号）の機械的マッチング手順を tip 化
    - **典型誤判定の warning**: (a)「電動機損壊だから第3号」誤判定 — 第3号は「他の物件」要件で自社設備のみは非該当／(b)「落雷＝自然現象だから対象外」誤判定 — 判定は結果で行うので速報24h は必須／(c)「半焼程度では軽微」誤判定 — 半焼以上は半焼を含む／(d)「直接の通電感電でないと対象外」誤判定 — 第1号は誤操作によるアーク熱火傷等も含む
    - **セクション12 過去問実績 更新**: R06上問2／R07上問2 の形式を「論説」→「論説（該当判定5択）」に明確化し演習2/演習3 へのリンク誘導追加。H22 問3 は kakomon DB対象外（H23-R07）だが R06上/R07上 の原型として行追加
    - **典拠**: denken-ou.com（H22問3／R06上問2／R07上問2 ページ）一次照合 2026-05-19。電気関係報告規則 第3条第1項表（eGov lawid=340M50000400054・最新版/令和7年改正準拠）と機械的に照合
- **2026-06-10 v2.13 H26問2 を第3条実績から除外（出題回数 6→5 訂正）**: kijun/58.md の虚偽引用横断監査で発見。v2.0（2026-05-05）で「H26問3→問2・5回→6回」と修正した際、H26問2 を第3条（事故報告）の出題実績として誤計上していた。denken-ou（houkih26-2）一次照合の結果、**H26問2 は報告規則第5条**（自家用設置者の出力・電圧変更／廃止の報告・正解=出力/電圧/廃止）で第3条とは別論点と確定。第3条の実出題は **R02問2／R05上問2／R06上問2／R07上問2／R07下問2 の5回**。出題頻度メタ・直近出題・黄色マーカー根拠表（事故／産業保安監督部長の H26問2 引用）・数値検証PASSテーブル・過去問実績表・出題傾向 note・フッタを 6→5 に一括訂正。kakomon.yml の H26問2 article（"報告規則"）は第5条相当で正しいため無改変。条文知識・図解・演習等の gold 資産は無改変。
...
*最終確認: 2026-06-10 | ステータス: v2.13（H26問2を第5条と確定し第3条実績を6→5に訂正）｜ 条文番号: ✅ 一次ソース照合済（eGov 2026-05-10 第3条第1項本文＋表 第1〜14号＋第2項本文）｜ 出題実績: ✅ kakomon DB 5件〔報告規則第3条明示〕+ H22問3（DB対象外） 照合済（H26問2は第5条のため除外・2026-06-10 denken-ou 照合）｜ 数値根拠: ✅ 速報24h／詳報30日＝第3条第2項本文／蓄電池の事故報告閾値＝第4号ト（20kWh超 破損）・第7号（10万kW以上 7日以上 放電支障）｜ [バージョニング基準](../../reference/versioning.md)*
