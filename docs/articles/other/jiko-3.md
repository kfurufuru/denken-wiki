...
| **第7号の閾値** | **10万kW以上** の蓄電所での **7日以上** の放電支障事故（第7号 条文文言） |
| **規模区分（参考）** | 工事計画届出の対象は **容量八万キロワットアワー（80,000kWh = 80MWh）以上** の電力貯蔵装置（電気事業法施行規則 別表 第2 — eGov 一次照合 2026-05-10）。事故報告閾値（第7号 出力10万kW以上）とは別の規模基準なので混同しない |
| **試験での扱い** | 「蓄電池・電力貯蔵装置は報告対象 **外**」という選択肢は **明確に誤り**。最新改正で第7号（放電支障事故）等として対象に明確化済み |

!!! warning "「蓄電池・電力貯蔵装置は対象外」は誤りで頻出ひっかけ"
    R07以降の最新改正論点。古い教材を使った受験者が「蓄電池は条文に出てこないから対象外」と判断しがちだが、**第3条第1項表 第7号「放電支障事故」が蓄電所を直接の対象としている**。選択肢に「蓄電池・電力貯蔵装置の事故は本規則の対象外」という記述があれば、**それは誤り** と判定する。

!!! note "20kWh 等の数値情報を見たら"
    インターネット上には「容量20kWh超で報告対象」のような数値情報が散見されるが、**eGov 一次照合（2026-05-10）では電気事業法施行規則本文に20kWhの基準は確認できなかった**。施行規則別表で確認できるのは **80,000kWh以上**（工事計画届出対象）と **第3条第1項表 第7号**（出力10万kW以上で7日以上の放電支障）の2基準。試験対策上は **第7号の「10万kW以上・7日以上」** を覚えれば十分。それ以外の細かい容量数値（20kWh等）は出題実績ゼロ・記憶不要。
...
*最終確認: 2026-05-10 | ステータス: v2.8（電話等表現精緻化・穴埋め問題3再構築・時/日起算明示・義務者限定・80,000kWh基準を一次照合補足）｜ 条文番号: ✅ 一次ソース照合済（eGov 2026-05-10 第3条第1項表＋第2項本文・施行規則 lawid=407M50000400077 別表）｜ 出題実績: ✅ kakomon DB 6件照合済（H26/R02/R05上/R06上/R07上/R07下）｜ 数値根拠: ✅ 速報24h／詳報30日＝**第3条第2項本文**／蓄電池基準＝施行規則別表で 80,000kWh以上（一次照合）｜ [バージョニング基準](../../reference/versioning.md)*
