...
> この法律は、<mark>電気工事の作業に従事する者</mark>の<mark>資格及び義務</mark>を定め、もつて<mark>電気工事の欠陥による災害の発生の防止</mark>に<mark>寄与</mark>することを目的とする。

> <small>黄色マーカー = 過去問・予想問題で穴埋め対象になりうるキーワード</small>

→ 第1条は **「手段（資格・義務を定める）」→「目的（災害防止に寄与する）」** という典型的な目的規定の構文。条文中に数値・手続規定はなく、第2条以下の各条文がこの目的を実現する手段を具体化する構造になっている。

### 過去問で問われた箇所

第1条の単独出題は過去14年で実質1回（H20問8の選択肢として「電気工事士法の目的」が登場）。穴埋めではなく、**第3条・第5条と組み合わせた論説問題のリード文** として参照されるパターンが中心。

| 出題で狙われうるキーワード | 想定される問われ方 |
|---|---|
| **電気工事の作業に従事する者** | 「義務の対象者は誰か」を問う |
| **資格及び義務** | 法律の手段としての2本柱を問う |
| **電気工事の欠陥** | 災害の原因を限定する語（事故全般ではない） |
| **災害の発生の防止** | 法律の最終目的を問う（「地位向上」等のひっかけ選択肢に注意） |
| **寄与** | 「防止する」ではなく「寄与する」（断定を避ける法律用語）|
...
- [電気関係報告規則 第1条：目的](jiko-1.md) — 比較対象（同じ「目的条文」の構造）
