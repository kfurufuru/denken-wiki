...
!!! note "📊 試験対策メタ"
    ...
    **直近出題**: kakomon.yml（H23〜R07）で電気工事士法の登録は H26 問3／H29 問2／R06下 問2 の3件（いずれも第1条・第3条）。**第2条単独の出題実績は未確認**
...
| **典拠（一次ソース）** | 電気工事士法（昭和35年法律第139号）第2条 — 本文は照合済 ／ **同法施行令第1条（軽微な工事の具体例）は未照合**（下記 honest-hold） |
...
!!! danger "honest-hold: 「軽微な工事」の数値（30VA・36V 等）は一次照合していない"
    本ページに現れる **30VA**（小型変圧器）・**36V**（蓄電池）・電球交換・差込接続器などの列挙は、**電気工事士法施行令第1条の原文と突き合わせていない**（本環境から e-Gov に到達できないため）。学習教材で広く流通している値だが、**本 Wiki が一次照合した数値ではない**。

    - **確実なのは第2条第3項の建付け**＝「政令で定める軽微な工事を除く」という**委任の構造**まで。ここは条文原文で照合済み。
    - 数値そのものを問う出題に備えるなら、**施行令第1条の原文（e-Gov）を各自で確認**すること。
...
??? question "セルフチェック: 第一種電気工事士は800kWの自家用工作物の工事ができるか？"
    **できない**。電気工事士法第2条の「自家用電気工作物」は **500kW未満** に限定されており、500kW以上は本法の対象外。ただし無資格で工事できるわけではなく、**電気事業法第43条で選任される電気主任技術者の監督下** で工事する自主保安体系に切り替わる（詳細は 第6条 を参照）。第一種でも500kW以上は工事士法の射程外。**なお R05上問3 は電技省令第14条（過電流保護）の出題であり本条の実績ではない**（2026-08-28 kakomon.yml 突合で訂正）。
...
| キーワード | 試験での扱い |
|---|---|
| 軽微な工事の具体例 | 出題実績は kakomon.yml 未登録（H26問3・H29問2・R06下問2 はいずれも第1条・第3条） |
| 一般用電気工作物 vs 自家用電気工作物の境界 | 電気事業法第38条側（jigyoho/38.md）で頻出 |
| 500kW境界 | 出題実績は未確認 |
| 小型変圧器の二次側配線等「軽微な工事」の範囲 | **施行令第1条の一次照合が未了**（本コンテナから参照不能）。数値は要確認
...
- **試験のポイント**: 単純な「一般用」ではなく **「等」付き**。小規模事業用（太陽電池発電設備は出力 **10kW以上50kW未満**・電気事業法第38条第3項）を含む。**10kW未満は一般用電気工作物**であって小規模事業用ではない（出力が省令で定める出力「未満」＝一般用／「以上」＝小規模事業用・同条第1項第二号イ、第3項第一号イ）
...
??? question "セルフチェック: 出力5kWの家庭用太陽光発電を設置する工事は何条の対象か？"
    **電気工事士法第2条の電気工事（一般用電気工作物等）**。ただし区分に注意：出力5kW（**10kW未満**）の太陽光は小規模発電設備であり **「一般用電気工作物」そのもの**（電気事業法第38条第1項第二号イ「出力が経済産業省令で定める出力**未満**」）。小規模事業用電気工作物は **10kW以上50kW未満**。どちらも「一般用電気工作物**等**」に含まれるため第2条の射程だが、区分名を取り違えないこと。第二種電気工事士で作業可能。
...
??? note "解説と復習導線"
    「ア＝施行令への政令委任」は条文原文で照合済みで、ここが本問の核心。
...
- **2026-08-28 監査是正**（ChatGPT/Fable 監査 → Claude 一次照合）: ①「小規模事業用＝出力10kW未満」を是正（電気事業法第38条第1項第二号イ「省令で定める出力**未満**」＝一般用／同第3項第一号イ「**以上**」＝小規模事業用。太陽電池は10kW未満が一般用、10kW以上50kW未満が小規模事業用）。②出題実績 R05上問3・H29問4・H27問3 を撤去（kakomon.yml 突合でいずれも電技省令系の出題と判明）。③軽微な工事の数値（36V／30VA）は施行令の一次照合が本環境で不能のため honest-hold に変更。
