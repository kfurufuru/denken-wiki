...
| 連系区分 | 電圧レベル | 代表例 | 根拠条文（施設要件／保護装置） |
|---------|----------|--------|---------|
| **低圧連系（単相3線式）** | ==100V== / ==200V== | 住宅用太陽光（10kW未満） | 施設要件=第226条／保護装置=第227条 |
| **低圧連系（三相3線式）** | ==200V== | 小規模風力・小型水力 | 施設要件=第226条／保護装置=第227条 |
| **高圧連系** | ==6.6kV== | 産業用太陽光・バイオマス（数百kW〜数MW） | 施設要件=第228条／保護装置=第229条／再閉路防止=第224条 |
| **特別高圧連系（22〜66kV）** | ==22kV==〜==66kV== | 大規模太陽光・風力（数MW〜） | 施設要件=第230条／保護装置=第231条 |
| **特別高圧連系（154kV以上）** | ==154kV==〜 | 超大型風力・大規模水力 | 施設要件=第230条／保護装置=第231条 |
| **共通（全区分）** | — | — | 用語定義=第220条／直流流出防止=第221条／限流リアクトル=第222条／自動負荷制限=第223条／電話設備=第225条／例外=第232条 |
...
| **周波数** | ==50Hz==±==0.2Hz== または ==60Hz==±==0.2Hz==（連系時） | 系統連系規程 JEAC 9701（OFR/UFR 整定の目安） |
...
│
├─ 低圧（100V/200V）
│   └─ → 低圧連系（施設要件 第226条／保護装置 第227条）
│
├─ 高圧（==6.6kV==）
│   ├─ スポットネットワーク受電方式？ → 第224条（再閉路防止）は適用除外に注意
│   └─ → 高圧連系（施設要件 第228条／保護装置 第229条／再閉路防止 第224条）
│
└─ 特別高圧（==22kV==以上）
    └─ → 特別高圧連系（施設要件 第230条／保護装置 第231条）
```
