...
設備利用率は 第3条 全日効率の負荷率と数式構造が同じ：

- 負荷率（第3条）= 平均需要電力 ÷ 最大需要電力
- 設備利用率（第7条-1）= 平均出力 ÷ 最大（定格）出力
...
!!! note "R06上期問13の鍵"
    Ig=5A、0.8秒以内自動遮断 → 基準 = 600/5 = 120Ω。「許容値の1/3に維持」→ R_B = 120 × 1/3 = **40**Ω。  
    人体: V_safe = 6000 × 0.01 = 60V。並列回路で V_外箱 = 100 × X/(X+40) ≦ 60、X = R_D × 6000/(R_D + 6000) → 計算結果 R_D ≦ 60.6Ω → **60**Ω。
