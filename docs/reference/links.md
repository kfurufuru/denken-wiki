...
*最終更新: 2026-06-11（セクション3を「民間規程・技術規格」/セクション4を「試験情報・過去問・学習リソース」へ見出し拡張＝refs カテゴリ照合先の整備・issue #58／AC0 誤記 LawId の重複 bullet を削除／§3 JEAC の404リンク（表と重複）を除去し、bullet が見出し化していた箇所の空行を補完（merge_refs.py 挿入バグ修正）。前回: 2026-05-10 リファクタで7セクション構造へ再編）*
