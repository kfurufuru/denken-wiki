...
| **届出** | 事前 | 受理後**30日経過で着工可** | 「**通知して把握してもらう**」だけ | 工事計画届出（電事法第48条）|
...
| 誤った理解 | 正しい理解 |
|-----------|-----------|
| 工事計画はすべて「届出」 | **第47条＝認可（認可まで着工不可）**／**第48条＝届出（受理+30日で着工可）** |
| 届出は「30日前までに出す」 | 条文は「**届出受理後30日を経過後に着工可**」（実務上の感覚は同じだが条文表現は後者）|
...
*最終更新: 2026-05-05 | v1.1（条文表現に修正・Mermaid可視化・自己チェック追加）*
