...
|------|----------|--------------------------|----------|------------------|
| **電気事業法** | 経済産業大臣 | 工事計画認可§47①／工事計画届出§48①（**受電電圧1万V以上の需要設備等**）／保安規程届出§42②／主任技術者選任届出§43⑤ | ★★★ | 設備の安全性・公益性は**国レベル**で管理 |
...
| **主任技術者選任届出**（電事法§43⑤） | **法令上の宛先＝経済産業大臣** | **提出窓口は所轄産業保安監督部でOK**（省令委任）。宛先＝大臣は変わらない |
...
- 条文番号：✅ 確認済
- 電気工事業法の宛先分岐：✅ 3分岐に修正済（1県内／同一監督部内複数県／複数監督部にまたがる）
- 需要設備の工事計画届出対象：✅ 受電電圧1万V以上に修正済
- 蓄電池・電力貯蔵装置の事故報告対象：✅ 令和7年11月20日施行改正を反映済
- 「法令上の宛先」と「提出窓口」の概念分離：✅ 明示済
- **次回確認**: 令和8年度試験公示後

---

*ステータス: v1.1 ｜ 条文番号: ✅ 一次ソース照合済 ｜ 出題実績: ✅ kakomon DB 照合済（宛先関連15問）｜ 法令正確性: ✅ ChatGPT指摘6点反映済 ｜ [バージョニング基準](versioning.md)*
