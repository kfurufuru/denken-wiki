...
RULES: list[Rule] = [
    ...
    Rule(
        id="H04",
        description="単相3線式200Vを0.2MΩと断定（中性線接地で対地100V→正しくは0.1MΩ）",
        patterns=[
            r"単相3線式.{0,12}200\s*V?.{0,20}0\.2\s*MΩ",
            r"0\.2\s*MΩ.{0,20}単相3線式.{0,12}200",
        ],
        exclude_contexts=[
            "警告", "warning", "誤り", "❌", "三相", "0.1", "Y結線",
            "ではない", "✗", "比較", "対比", "混同", "誤",
        ],
        severity="WARNING",
        note=(
            "単相3線式200Vは中性線接地のため対地電圧100V（線間の半分）→ 0.1MΩ（ゾーン1）。"
            "0.2MΩは三相3線式200V（対地200V扱い）の値。線間200Vだけ見て0.2と早合点しない。"
            "（value-correction-sweep: kijun/58 で本文/SVG/表のいずれかに訂正漏れが残ると発火）"
        ),
    ),
    Rule(
        id="H05",
        description="三相3線式200Vを0.1MΩと断定（Δ結線で対地200V→正しくは0.2MΩ）",
        patterns=[
            r"三相3線式.{0,12}200\s*V?.{0,20}0\.1\s*MΩ",
            r"0\.1\s*MΩ.{0,20}三相3線式.{0,12}200",
        ],
        exclude_contexts=[
            "警告", "warning", "誤り", "❌", "単相", "中性点", "Y結線",
            "ではない", "✗", "比較", "対比", "混同", "0.2", "誤",
        ],
        severity="WARNING",
        note=(
            "三相3線式200V（Δ結線・一般低圧動力）は対地電圧200V扱い → 0.2MΩ（ゾーン2）。"
            "0.1MΩは単相3線式（対地100V）の値。三相200Vを単相と取り違えて0.1としない。"
            "（value-correction-sweep: kijun/58 で本文/SVG/表のいずれかに訂正漏れが残ると発火）"
        ),
    ),
]
