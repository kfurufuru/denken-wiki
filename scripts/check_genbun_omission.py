...
ALLOWLIST: dict[str, str] = {
    ...
    # 解釈第1条も同型。最大使用電圧の根拠になる第一号・第二号（イロハ）だけを逐語転記し、
    # 号三以降（技術員・電気使用場所・需要場所・引込線 等）は本文の「定義（要点）」表で扱う。
    # 2026-09-08 に第二号を転記したのは「最大使用電圧＝公称電圧×1.15/1.1」の根拠を機械照合
    # 可能にするためで、19号すべての逐語化は別スコープ（解消するときはこの行を消す）。
    "docs/articles/kaishaku/1.md:第1条:OMIT:号三,四,五,六,七,八,九,十,十一,十二,十三,十四,十五,十六,十七,十八,十九":
        "第一号・第二号のみ逐語引用（最大使用電圧の根拠）。号三以降の定義は要点表（要約）で扱う設計。逐語未照合の債務として可視化",
}
