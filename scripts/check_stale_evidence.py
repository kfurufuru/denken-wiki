...
メタ表記（denken-wiki 標準・いずれの形式も照合日として拾う）:
  テーブル形式  : | **照合日** | 2026-05-02 |  ／  | **次回監査** | 2026-11-02 |（任意）
  インライン形式: - **照合日**: 2026-05-02（補足コメント）
  フッター形式  : *最終確認: 2026-05-02 | ステータス: v2.7 | ...*（照合日が無い場合の fallback）
...
# 照合日: テーブル形式「| **照合日** | YYYY-MM-DD |」と
# インライン形式「- **照合日**: YYYY-MM-DD」の両方を拾う（区切りは | : ：）
VERIFIED_RE = re.compile(
    r"\*\*照合日\*\*\s*[|:：]\s*(\d{4}-\d{2}-\d{2})"
)
