...
KNOWN_OK_MISMATCH: dict = {
    ...
    ("R03", 1): {
        "recorded_num": "57",
        "reason": (
            "調査の義務の多条文出題（denken-ou 明記: 『電気事業法第57条及び電気設備の"
            "技術基準の解釈第14条からの出題』）。yml の『事業法§57, 解釈§14』は正で"
            "代表条文は事業法§57（電線路維持運用者の調査義務）。簡易パースは解釈フル"
            "ネーム初出（第14条）を主表示として拾うため §14 が出る。H23問4 と同型。"
        ),
    },
    ("R03", 12): {
        "recorded_num": "15",
        "reason": (
            "高圧ケーブル交流絶縁耐力試験（解釈第15条＝最大使用電圧の1.5倍を10分）の"
            "計算問題。H28問12 と同型で、denken-ou は計算の前提となる最大使用電圧"
            "（解釈第1条の定義・1.15/1.1係数）を先に解説するため第1条を主表示として"
            "拾う。出題本体は §15 で yml は正。"
        ),
    },
    ...
    ("H24", 5): {
        "recorded_num": "5",
        "reason": (
            "電路の絶縁の多条文出題（省令§5＋解釈§13）。denken-ou は解釈第13条"
            "（電路の絶縁の例外列挙）を主表示するため §13 を拾うが、これは yml の"
            "2番目の条文そのもの。空欄出典は kijun/58 根拠表監査で省令第5条＋解釈第13条"
            "と実証済み（2026-06-08 確定マップ）。代表条文は省令 §5（yml 先頭）で正。"
        ),
    },
    ("H26", 5): {
        "recorded_num": "26",
