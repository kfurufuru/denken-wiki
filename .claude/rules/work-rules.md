...
以下3フィールドのいずれか一方を必ず出力。複数併記（種別 hedge＝updated+skipped 等の混在で逃げる）・全部なしは禁止、自由文形式は禁止。ただし **学びが複数あるときは `memory_updated`/`memory_created` 行を学びの数だけ併記してよい（1学び1行・同種なら複数可）**＝「併記禁止」は種別 hedge を禁じる趣旨であって、学びの取りこぼしを許す意味ではない（[feedback_task_retrospective_protocol]「検証の穴」節・2026-07-15 secretary 側 §7 と同期）。
