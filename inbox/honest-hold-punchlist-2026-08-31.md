...
## 法令ではない残件が 8件ある（本 PR 本文の要約の訂正）

PR 本文の「残るもの」は残件を**法令の条文原文だけ**のように書いていたが、実測すると
**H（過去問の原本）4件・I（試験センターの配点公表資料）4件**が含まれる。この8件は e-Gov では解けない。

- **H** … 出題実績の主張を撤回して honest-hold にした箇所。解消には**試験センター公表の問題冊子**か
  それに準ずる一次資料が要る（`kakomon.yml` は SoT だが、問題文の逐語までは持たない）。
- **I** … B問題の配点内訳（14/14/12 か 14/13/13 か）。解消には**試験センターの公表資料**が要る。
  repo 収録の実問題から確定しているのは「A問題6点・B問題14点」の2点のみ。
...
陽性対照: 上のコマンドが 0件を返したら**抽出器の故障を疑う**（本ファイル生成時点で 55件）。
